class Blind(Moveable):
    ...
    def __init__(self, X, Y, W, H):Moveable.init(self, X, Y, W, H)
        self.children = {}
        self.config = {}
    ...
    def set_text(self):
        if self.config.blind:
            if self.disabled:
                ...
                '1[self.loc_debuff_lines]' = ''
                '2[self.loc_debuff_lines]' = ''
            else:
                loc_vars = 'None'
                if self.name == 'The Ox':
                    loc_vars = {1: localize()}
                ...
                if loc_target:
                    ...
                    '1[self.loc_debuff_lines]' = '1[loc_target]' if '1[loc_target]' else ''
                    '2[self.loc_debuff_lines]' = '2[loc_target]' if '2[loc_target]' else ''
                else:
                    self.loc_name = ''';'
                    self.loc_debuff_text = ''
                    '1[self.loc_debuff_lines]' = ''
    ...
    def set_blind(self, blind, reset, silent):
        if not reset:
            ...
            if G.GAME.modifiers.no_blind_reward & 'self:get_type()[G.GAME.modifiers.no_blind_reward]':
                self.dollars = 0
    ...
    def disable(self):
        ...
        if ((self.name == 'The Wheel' if self.name == 'The Wheel' else self.name == 'The House') if (self.name == 'The Wheel' if self.name == 'The Wheel' else self.name == 'The House') else self.name == 'The Mark') if ((self.name == 'The Wheel' if self.name == 'The Wheel' else self.name == 'The House') if (self.name == 'The Wheel' if self.name == 'The Wheel' else self.name == 'The House') else self.name == 'The Mark') else self.name == 'The Fish':
            for i in range(1, len(G.hand.cards), ):
                if 'i[G.hand.cards]'.facing == 'back':'i[G.hand.cards]'.flip()
            for kv in pairs():
    ...
    def debuff_hand(self, cards, hand, handname, check):
        ...
        if self.debuff:
            ...
            if self.name == 'The Eye':
                if 'handname[self.hands]':
                    self.triggered = True
                    return True
                if not check:
                    'handname[self.hands]' = True
        ...
        if self.name == 'The Arm':
            self.triggered = False
            if 'handname[G.GAME.hands]'.level > 1:
                self.triggered = True
    ...
    def drawn_to_hand(self):
        if not self.disabled:
            ...
            if (self.name == 'Crimson Heart') & self.prepped & '1[G.jokers.cards]':
                jokers = {}
                for i in range(1, len(G.jokers.cards), ):
                    if not 'i[G.jokers.cards]'.debuff if not 'i[G.jokers.cards]'.debuff else len(G.jokers.cards < 2):
                        '#jokers + 1[jokers]' = 'i[G.jokers.cards]''i[G.jokers.cards]'.set_debuff(False)
    ...
    def move(self, dt):Moveable.move(self, dt)self.align()
    ...
    def load(self, blindTable):
        self.config.blind = 'blindTable.config_blind[G.P_BLINDS]' if 'blindTable.config_blind[G.P_BLINDS]' else {}
        ...
        if 'blindTable.config_blind[G.P_BLINDS]':
            self.blind_set = True
