...
if G.GAME.modifiers.no_blind_reward & (G.GAME.blind & G.GAME.modifiers.no_blind_reward[G.GAME.blind:get_type()]):
    if e.config.minh > 0.44:
        e.config.minh = 0.4
        G.GAME.modifiers.no_blind_reward[G.GAME.blind:get_type()].config.text = localize(k_no_reward)e.UIBox.recalculate(True)
elif e.config.minh < 0.45:
    e.config.minh = 0.45
    G.GAME.modifiers.no_blind_reward[G.GAME.blind:get_type()].config.text = localize(k_reward) + ': '
    G.GAME.modifiers.no_blind_reward[G.GAME.blind:get_type()].states.visible = Truee.UIBox.recalculate(True)
...
if G.PROFILES[G.SETTINGS.profile[G.PROFILES]'.all_unlocked if 'G.SETTINGS.profile].all_unlocked else e.config.disable_button:
    e.config.colour = G.C.UI.BACKGROUND_INACTIVE
    e.config.button = 'None'
else:
...
def toggle_button(e):
    e.config.ref_table.ref_table[e.config.ref_table.ref_value[e.config.ref_table.ref_table]' = not 'e.config.ref_table.ref_value]
    if e.config.toggle_callback:e.config.toggle_callback(e.config.ref_table.ref_value[e.config.ref_table.ref_table])
def toggle(e):
    if (not e.config.ref_table.ref_table[e.config.ref_table.ref_value]) & e.config.toggle_active:
        e.config.toggle_active = 'None'
        e.config.colour = e.config.ref_table.inactive_colour
        e.config.ref_table.ref_table[e.config.ref_table.ref_value].states.visible = False
        e.config.ref_table.ref_table[e.config.ref_table.ref_value].config.object.states.visible = False
    elif e.config.ref_table.ref_table[e.config.ref_table.ref_value] & (not e.config.toggle_active):
        e.config.toggle_active = True
        e.config.colour = e.config.ref_table.active_colour
        e.config.ref_table.ref_table[e.config.ref_table.ref_value].states.visible = True
        e.config.ref_table.ref_table[e.config.ref_table.ref_value].config.object.states.visible = True
def slider(e):
    c = e.children[1]
    ...
    if G.CONTROLLER & G.CONTROLLER.dragging.target & (G.CONTROLLER.dragging.target == e if G.CONTROLLER.dragging.target == e else G.CONTROLLER.dragging.target == c):
        ...
        e.children[1] = math.min()
        rt.text = string.format()
        c.T.w = (e.children[1] - rt.min) / (rt.max - rt.min) * rt.w
        c.config.w = c.T.w
        if rt.callback:e.children[1](rt)
...
from_val = e.config.ref_table.options[e.config.ref_table.current_option]
...
to_val = e.config.ref_table.options[e.config.ref_table.current_option]
...
if e.config.ref_table.opt_callback:e.config.ref_table.options[e.config.ref_table.current_option]()
...
for j in range(1, len(G.your_collection), ):
    for i in range(len(G.your_collection[j].cards), 1, -1):
        c = G.your_collection[j].remove_card(i[G.your_collection[j].cards])c.remove()
        c = 'None'
for i in range(1, 5, ):
    for j in range(1, len(G.your_collection), ):
...
to_resolution = curr_disp].screen_resolutions.values[args.to_key[G.SETTINGS.WINDOW.DISPLAYS]
...
G.SETTINGS.QUEUED_CHANGE.screenmode = G.ARGS.screenmode_vals[args.to_key]G.FUNCS.change_window_cycle_UI()
...
if G.OVERLAY_MENU:
    ...
    if swap_node:
        ...
        res_option = GET_DISPLAYINFO(focused_screenmode, focused_display)swap_node.children[1].remove()
        swap_node.children[1] = 'None'swap_node.UIBox.add_child()
...
if G.CONTROLLER.text_input_hook == e:
    ...
    args.current_prompt_text = (args.text.ref_table[args.text.ref_value[args.text.ref_table]' == '') & args.prompt_text if ('args.text.ref_value] == '') & args.prompt_text else ''
...
G.CONTROLLER.text_input_hook = 1].children[1[e.children]TRANSPOSE_TEXT_INPUT(0)e.UIBox.recalculate(True)
G.FUNCS.text_input_key = lambda args: 
args = args if args else {}
if args.key == ' else args.key == '[[' if args.key == ']:
    return False
if args.key == '0':
    args.key = 'o'
...
args.key = ' else args.key == '[[' if args.key == '] else args.caps & string.upper() if args.caps & string.upper() else args.keyTRANSPOSE_TEXT_INPUT(0)
if (string.len(text.ref_value[text.ref_table]) > 0) & (args.key == 'BACKSPACE'):MODIFY_TEXT_INPUT()TRANSPOSE_TEXT_INPUT()
    ...
    ' else args.key == '[[' if args.key == ']
    ' else args.key == '[[' if args.key == ']
    ' else args.key == '[[' if args.key == ']ease_colour()
...
def GET_TEXT_FROM_INPUT():
    ...
    for i in range(1, len(hook.children), ):
        if hook.children[i[hook.children]'.config & ('i[hook.children]'.config.id.sub(1, 7) == 'letter_') & ('i].config.text != ''):
            new_text = new_text + hook.children[i[hook.children]'.config & ('i[hook.children]'.config.id.sub(1, 7) == 'letter_') & ('i].config.text
    return new_text
def MODIFY_TEXT_INPUT(args):
    ...
    if args.delete & (args.pos > 0):
        if args.pos >= len(args.text_table.letters):
            ...
            args.text_table.letters[args.pos]MODIFY_TEXT_INPUT()
        return False
    swapped_letter = args.text_table.letters[args.pos]
...
def TRANSPOSE_TEXT_INPUT(amount):
    ...
    for i in range(1, len(hook.children), ):
        if hook.children[i].config:
            if hook.children[i].config.id == 'position':
                position_child = i';'
                break
...
G.SETTINGS.WINDOW.DISPLAYS[G.SETTINGS.WINDOW.selected_display].screen_res = {'w': (G.SETTINGS.QUEUED_CHANGE & G.SETTINGS.QUEUED_CHANGE.screenres & G.SETTINGS.QUEUED_CHANGE.screenres.w if G.SETTINGS.QUEUED_CHANGE & G.SETTINGS.QUEUED_CHANGE.screenres & G.SETTINGS.QUEUED_CHANGE.screenres.w else G.SETTINGS.screen_res & G.SETTINGS.screen_res.w) if (G.SETTINGS.QUEUED_CHANGE & G.SETTINGS.QUEUED_CHANGE.screenres & G.SETTINGS.QUEUED_CHANGE.screenres.w if G.SETTINGS.QUEUED_CHANGE & G.SETTINGS.QUEUED_CHANGE.screenres & G.SETTINGS.QUEUED_CHANGE.screenres.w else G.SETTINGS.screen_res & G.SETTINGS.screen_res.w) else love.graphics.getWidth(), 'h': (G.SETTINGS.QUEUED_CHANGE & G.SETTINGS.QUEUED_CHANGE.screenres & G.SETTINGS.QUEUED_CHANGE.screenres.h if G.SETTINGS.QUEUED_CHANGE & G.SETTINGS.QUEUED_CHANGE.screenres & G.SETTINGS.QUEUED_CHANGE.screenres.h else G.SETTINGS.screen_res & G.SETTINGS.screen_res.h) if (G.SETTINGS.QUEUED_CHANGE & G.SETTINGS.QUEUED_CHANGE.screenres & G.SETTINGS.QUEUED_CHANGE.screenres.h if G.SETTINGS.QUEUED_CHANGE & G.SETTINGS.QUEUED_CHANGE.screenres & G.SETTINGS.QUEUED_CHANGE.screenres.h else G.SETTINGS.screen_res & G.SETTINGS.screen_res.h) else love.graphics.getHeight()}
...
if G.SETTINGS.current_setup == 'New Run':
    if not G.GAME if not G.GAME else (not G.GAME.won) & (not G.GAME.seeded):
        if G.SAVED_GAME != 'None':
            if not G.SAVED_GAME.GAME.won:
                G.PROFILES[G.SETTINGS.profile].high_scores.current_streak.amt = 0G.save_settings()
    ...
    _stake = (G.forced_stake if G.forced_stake else G.PROFILES[G.SETTINGS.profile].MEMORY.stake) else 1G.FUNCS.start_run(e)
...
for kv in pairs():
    if e.config.id == v.id:
        if not e.config.object.is(Sprite) if not e.config.object.is(Sprite) else e.config.object.ID != v.ID:e.config.object.remove()
            ...
            G.C.UI_CHIPLICK[i] = {'intensity': 0, 'real_intensity': 0, 'intensity_vel': 0, 'colour_1': v.colour, 'colour_2': v.accent, 'timer': G.TIMERS.REAL}e.config.object.set_alignment()e.config.object.define_draw_steps()e.config.object.get_pos_pixel()
        _F = G.C.UI_CHIPLICK[i]
        exptime = math.exp()
...
if G.pack_cards & G.hand.cards[1[G.pack_cards.cards]' & (((G.STATE == G.STATES.PLANET_PACK if G.STATE == G.STATES.PLANET_PACK else G.STATE == G.STATES.STANDARD_PACK) if (G.STATE == G.STATES.PLANET_PACK if G.STATE == G.STATES.PLANET_PACK else G.STATE == G.STATES.STANDARD_PACK) else G.STATE == G.STATES.BUFFOON_PACK) if ((G.STATE == G.STATES.PLANET_PACK if G.STATE == G.STATES.PLANET_PACK else G.STATE == G.STATES.STANDARD_PACK) if (G.STATE == G.STATES.PLANET_PACK if G.STATE == G.STATES.PLANET_PACK else G.STATE == G.STATES.STANDARD_PACK) else G.STATE == G.STATES.BUFFOON_PACK) else G.hand & ('1[G.hand.cards]' if '1] else G.hand.config.card_limit <= 0)):
    e.config.colour = G.C.GREY
...
if card.ability.consumeable:
    ...
    if card.ability.booster_pos:
        G.jokers.cards[i] = 'USED'draw_card()
...
for i in range(1, len(G.jokers.cards), ):
    if G.jokers.cards[i[G.jokers.cards]' != card:'i].calculate_joker()
...
if G.shop_vouchers & G.shop_vouchers.cards & (G.shop_vouchers.cards[1[G.shop_vouchers.cards]' if '1] else G.GAME.current_round.voucher):
    e.states.visible = False
...
if G.shop:
    for i in range(1, len(G.jokers.cards), ):G.jokers.cards[i].calculate_joker()G.E_MANAGER.add_event()G.E_MANAGER.add_event()
...
for i in range(1, len(G.jokers.cards), ):G.jokers.cards[i].calculate_joker()G.FUNCS.end_consumeable(e)
...
if (not e.config.ref_table.run_info) & G.blind_select & G.blind_select.VT.y < 10 & e.config.id & G.blind_select_opts[string.lower(e.config.id)]:
    ...
    if (e.config.ref_table.deck != 'on') & (e.config.id == G.GAME.blind_on_deck) if (e.config.ref_table.deck != 'on') & (e.config.id == G.GAME.blind_on_deck) else (e.config.ref_table.deck != 'off') & (e.config.id != G.GAME.blind_on_deck):
        _blind_choice = G.blind_select_opts[string.lower(e.config.id)]
        _top_button = e.UIBox.get_UIE_by_ID(select_blind_button)
        _border = G.blind_select_opts[string.lower(e.config.id)]
        ...
        if e.config.id == G.GAME.blind_on_deck:
            ...
            if _tag & _tag_container:
                G.blind_select_opts[string.lower(e.config.id)].config.draw_after = False
                G.blind_select_opts[string.lower(e.config.id)].config.colour = G.C.BLACK
                G.blind_select_opts[string.lower(e.config.id)].config.button = 'skip_blind'
                _tag.config.outline_colour = adjust_alpha()
                G.blind_select_opts[string.lower(e.config.id)].config.hover = True
                G.blind_select_opts[string.lower(e.config.id)].config.colour = G.C.RED
                G.blind_select_opts[string.lower(e.config.id)].config.colour = G.C.UI.TEXT_LIGHT
            ...
            if _top_button:G.E_MANAGER.add_event()
                ...
                G.blind_select_opts[string.lower(e.config.id)].config.colour = G.C.WHITE
            ...
            if _tag & _tag_container:
                if G.blind_select_opts[string.lower(e.config.id)].align(0, 10)
                if G.blind_select_opts[string.lower(e.config.id)] == 'Skipped':
                    _blind_choice.children.alert = UIBox()
                G.blind_select_opts[string.lower(e.config.id)].config.button = 'None'
                _tag.config.outline_colour = G.C.UI.BACKGROUND_INACTIVE
                G.blind_select_opts[string.lower(e.config.id)].config.hover = False
                G.blind_select_opts[string.lower(e.config.id)].config.colour = G.C.UI.BACKGROUND_INACTIVE
                G.blind_select_opts[string.lower(e.config.id)].config.colour = G.C.UI.TEXT_INACTIVE
                _sprite = _tag.config.ref_table
...
if _tag:add_tag()
    ...
    G.GAME.round_resets.blind_states[skipped] = 'Select'
    G.GAME.blind_on_deck = skip_toplay_sound(generic1)G.E_MANAGER.add_event()
G.FUNCS.reroll_boss_button = lambda e: 
if (G.GAME.dollars - G.GAME.bankrupt_at - 10 >= 0) & (G.GAME.used_vouchers["v_retcon"[G.GAME.used_vouchers]' if '"v_retcon"[G.GAME.used_vouchers]' else '"v_directors_cut"] & (not G.GAME.round_resets.boss_rerolled)):
    e.config.colour = G.C.RED
    e.config.button = 'reroll_boss'
    G.GAME.used_vouchers["v_retcon"[G.GAME.used_vouchers]' if '"v_retcon"[G.GAME.used_vouchers]' else '"v_directors_cut"].config.shadow = True
    if G.GAME.used_vouchers["v_retcon"[G.GAME.used_vouchers]' if '"v_retcon"[G.GAME.used_vouchers]' else '"v_directors_cut"]:
        G.GAME.used_vouchers["v_retcon"[G.GAME.used_vouchers]' if '"v_retcon"[G.GAME.used_vouchers]' else '"v_directors_cut"].config.shadow = True
else:
    e.config.colour = G.C.UI.BACKGROUND_INACTIVE
    e.config.button = 'None'
    G.GAME.used_vouchers["v_retcon"[G.GAME.used_vouchers]' if '"v_retcon"[G.GAME.used_vouchers]' else '"v_directors_cut"].config.shadow = False
    if G.GAME.used_vouchers["v_retcon"[G.GAME.used_vouchers]' if '"v_retcon"[G.GAME.used_vouchers]' else '"v_directors_cut"]:
        G.GAME.used_vouchers["v_retcon"[G.GAME.used_vouchers]' if '"v_retcon"[G.GAME.used_vouchers]' else '"v_directors_cut"].config.shadow = False
...
if message:
    for kv in ipairs():v.children[1].config.object.pulse()G.E_MANAGER.add_event()
