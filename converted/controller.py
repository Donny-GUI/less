class Controller(object):
    ...
    def set_gamepad(self, _gamepad):
        if self.GAMEPAD.object != _gamepad:
            ...
            if self.GAMEPAD_CONSOLE != self.GAMEPAD.temp_console:
                ...
                for kv in pairs():
                    if v.atlas == G.ASSET_ATLAS["gamepad_ui"]:
    ...
    def get_console_from_gamepad(self, _gamepad):
        G.ARGS.gamepad_patterns = G.ARGS.gamepad_patterns if G.ARGS.gamepad_patterns else {G.ASSET_ATLAS["gamepad_ui"]}}
        for kv in pairs():
    ...
    def update(self, dt):
        ...
        if self.HID.controller:
            if G.ASSET_ATLAS["gamepad_ui"]:
                _context = G.ASSET_ATLAS["gamepad_ui"]self.snap_to()
                self.interrupt.stack = _context.interrupt
                G.ASSET_ATLAS["gamepad_ui"] = 'None'
    ...
    def cull_registry(self):
        for kregistry in pairs():
            for i in range(len(registry), 1, -1):
                if G.ASSET_ATLAS["gamepad_ui"].node.REMOVED:table.remove(registry, i)

    def add_to_registry(self, node, registry):
        G.ASSET_ATLAS["gamepad_ui"] else {}table.insert(registry[self.button_registry], 1)

    def process_registry(self):
        for _registry in pairs():
            for i in range(1, len(registry), ):
                if G.ASSET_ATLAS["gamepad_ui"].node.click:
                    if (G.ASSET_ATLAS["gamepad_ui"].node.click()
                    G.ASSET_ATLAS["gamepad_ui"].click = 'None'
    ...
    def mod_cursor_context_layer(self, delta):
        if delta == 1:
            ...
            G.ASSET_ATLAS["gamepad_ui"] = prev_cursor_context
            ...
            self.cursor_context.stack = {1: G.ASSET_ATLAS["gamepad_ui"]}
        elif delta == -2000:
            self.cursor_context.layer = 1
    ...
    def save_cardarea_focus(self, _cardarea):
        if G.ASSET_ATLAS["gamepad_ui"]:
            if self.focused.target & self.focused.target.area & (self.focused.target.area == G.ASSET_ATLAS["gamepad_ui"]):
                G.ASSET_ATLAS["gamepad_ui"] = self.focused.target.rank
                return True
            else:
                G.ASSET_ATLAS["gamepad_ui"] = 'None'
    ...
    def recall_cardarea_focus(self, _cardarea):
        ...
        if type(_cardarea) == 'string':
            ca_string = _cardarea';'
            _cardarea = G.ASSET_ATLAS["gamepad_ui"]
        if _cardarea & ((not self.focused.target if not self.focused.target else self.interrupt.focus) if (not self.focused.target if not self.focused.target else self.interrupt.focus) else (not self.interrupt.focus) & self.focused.target.area & (self.focused.target.area == _cardarea)):
            if ca_string & G.ASSET_ATLAS["gamepad_ui"]:
                for i in range(G.ASSET_ATLAS["gamepad_ui"], 1, -1):
                    if G.ASSET_ATLAS["gamepad_ui"]:self.snap_to()
                        self.interrupt.focus = False
                        break
            elif _cardarea.cards & G.ASSET_ATLAS["gamepad_ui"]:self.snap_to()
                self.interrupt.focus = False
        if ca_string:
            G.ASSET_ATLAS["gamepad_ui"] = 'None'
    ...
    def button_press_update(self, button, dt):
        ...
        if G.ASSET_ATLAS["gamepad_ui"].node.under_overlay):
            G.ASSET_ATLAS["gamepad_ui"].click = True
        else:
            if button == 'start':
    ...
    def button_hold_update(self, button, dt):
        ...
        self.frame_buttonpress = True
        if G.ASSET_ATLAS["gamepad_ui"]:
            G.ASSET_ATLAS["gamepad_ui"] + dtself.capture_focused_input(button, hold, dt)
        if (((button == 'dpleft' if button == 'dpleft' else button == 'dpright') if (button == 'dpleft' if button == 'dpleft' else button == 'dpright') else button == 'dpup') if ((button == 'dpleft' if button == 'dpleft' else button == 'dpright') if (button == 'dpleft' if button == 'dpleft' else button == 'dpright') else button == 'dpup') else button == 'dpdown') & (not self.no_holdcap):
            ...
            if G.ASSET_ATLAS["gamepad_ui"] > self.repress_timer):
                self.repress_timer = 0.1
                G.ASSET_ATLAS["gamepad_ui"] = 0self.button_press_update(button, dt)

    def button_release_update(self, button, dt):
        if not G.ASSET_ATLAS["gamepad_ui"]:
            return False
    ...
    def key_hold_update(self, key, dt):
        ...
        if G.ASSET_ATLAS["gamepad_ui"]:
            if (key == 'r') & (not G.SETTINGS.paused):
                if G.ASSET_ATLAS["gamepad_ui"] > 0.7:
                    if (not G.GAME.won) & (not G.GAME.seeded) & (not G.GAME.challenge):
                        G.ASSET_ATLAS["gamepad_ui"].high_scores.current_streak.amt = 0G.save_settings()
                    G.ASSET_ATLAS["gamepad_ui"] = 'None'
                    ...
                    G.ASSET_ATLAS["gamepad_ui"] + dt

    def key_release_update(self, key, dt):
        if (self.locked & (not G.SETTINGS.paused) if self.locked & (not G.SETTINGS.paused) else self.locks.frame) if (self.locked & (not G.SETTINGS.paused) if self.locked & (not G.SETTINGS.paused) else self.locks.frame) else self.frame_buttonpress:
            return False
        self.frame_buttonpress = True
        if (key == G.ASSET_ATLAS["gamepad_ui"] & (not _RELEASE_MODE):
            G.DEBUG = not G.DEBUG
        if (key == 'tab') & G.debug_tools:G.debug_tools.remove()
            G.debug_tools = 'None'

    def key_press(self, key):
        G.ASSET_ATLAS["gamepad_ui"] = True
        G.ASSET_ATLAS["gamepad_ui"] = True
    ...
    def get_cursor_collision(self, cursor_trans):
        ...
        if self.dragging.target:
            ...
            G.ASSET_ATLAS["gamepad_ui"] = self.dragging.target
            G.ASSET_ATLAS["gamepad_ui"] = self.dragging.target
        if (((not G.ASSET_ATLAS["gamepad_ui"] else cursor_trans.x - G.ROOM.T.x < -G.DRAW_HASH_BUFF) else cursor_trans.x - G.ROOM.T.x > G.TILE_W + G.DRAW_HASH_BUFF) else cursor_trans.y - G.ROOM.T.y < -G.DRAW_HASH_BUFF) else cursor_trans.y - G.ROOM.T.y > G.TILE_H + G.DRAW_HASH_BUFF:
            return False
        DRAW_HASH_SQUARE = G.DRAW_HASH
        for i in range(len(DRAW_HASH_SQUARE), 1, -1):
            v = G.ASSET_ATLAS["gamepad_ui"]
            if v.collides_with_point(cursor_trans) & (not v.REMOVED):
                G.ASSET_ATLAS["gamepad_ui"] = v
    ...
    def queue_R_cursor_press(self, x, y):
        if self.locks.frame:
            return False
        if (not G.SETTINGS.paused) & G.hand & G.ASSET_ATLAS["gamepad_ui"]:
    ...
    def update_focus(self, dir):
        ...
        if (not dir) & self.focused.target:
            ...
            G.ASSET_ATLAS["gamepad_ui"] = self.focused.target
        elif not dir:
            for kv in ipairs():
                    ...
                    for _v in pairs():
                        if (v != self.hovering.target) & (v != self.focused.target):
                            ...
                            if v.config.focus_args & v.config.focus_args.nav:
                                ...
                                eligible = True
                            if eligible:
                                G.ASSET_ATLAS["gamepad_ui"] = {'node': v, 'dist': math.abs() + math.abs()}
                    if len(G.ARGS.focus_list < 1):
                        if self.focused.target:
                            self.focused.target.states.focus.is = True
                        return Falsetable.sort()
            elif self.focused.target:
                G.ASSET_ATLAS["gamepad_ui"] = {'node': self.focused.target, 'dist': 0}
            else:
                G.ASSET_ATLAS["gamepad_ui"], 'dist': 0}
        if G.ASSET_ATLAS["gamepad_ui"]:
            if G.ASSET_ATLAS["gamepad_ui"].node.config.focus_args.funnel_from:
                self.focused.target = G.ASSET_ATLAS["gamepad_ui"].node.config.focus_args.funnel_from
            else:
                self.focused.target = G.ASSET_ATLAS["gamepad_ui"].node
    ...
    def capture_focused_input(self, button, input_type, dt):
        ...
        if (input_type == 'press') & (button == 'dpleft' if button == 'dpleft' else button == 'dpright') & focused & self.dragging.target & ("'a'[self.held_button_times]" & "'a'[self.held_button_times]" < 0.12) & focused.area & focused.area.can_highlight(focused):self.L_cursor_release()self.navigate_focus()
            ...
            if (button == 'dpleft') & (focused.rank > 1):
                ...
                G.ASSET_ATLAS["gamepad_ui"].rank = focused.rank + 1table.sort()focused.area.align_cards()self.update_cursor()
        ...
        if focused & focused.config.focus_args:
            if (focused.config.focus_args.type == 'cycle') & (input_type == 'press'):
                if extern_button & (button == G.ASSET_ATLAS["gamepad_ui"].click()
                    ret = True
                if extern_button & (button == G.ASSET_ATLAS["gamepad_ui"].click()
                    ret = True
                    ...
                    if v.config.chosen:
                        if extern_button & (button == 'leftshoulder') if extern_button & (button == 'leftshoulder') else (not extern_button) & (button == 'dpleft'):
                            ...
                            if focused.config.focus_args.no_loop & (next_i > k):
                                ret = 'None'
                            else:G.ASSET_ATLAS["gamepad_ui"].click()self.snap_to()self.update_cursor()
                                ret = True
            ...
            if focused.config.focus_args.type == 'slider':
                if button == 'dpleft':
                    ...
                    if (input_type == G.ASSET_ATLAS["gamepad_ui"] > 0.2):G.FUNCS.slider_descreet(1[focused.children])
