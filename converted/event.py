class Event(object):
    ...
    def __init__(self, config):
        ...
        if self.trigger == 'ease':
            self.ease = {config.ref_table[type': config.ease if config.ease else 'lerp', 'ref_table': config.ref_table, 'ref_value': config.ref_value, 'start_val': 'config.ref_value], 'end_val': config.ease_to, 'start_time': 'None', 'end_time': 'None'}
        ...
        if self.trigger == 'condition':
            ...
            self.func = config.func if config.func else lambda: 
            return config.ref_table[type': config.ease if config.ease else 'lerp', 'ref_table': config.ref_table, 'ref_value': config.ref_value, 'start_val': 'config.ref_value] == self.condition.stop_val
        self.time = config.ref_table[type': config.ease if config.ease else 'lerp', 'ref_table': config.ref_table, 'ref_value': config.ref_value, 'start_val': 'config.ref_value]
    ...
    def handle(self, _results):
        ...
        if self.trigger == 'after':
            if self.time + self.delay <= config.ref_table[type': config.ease if config.ease else 'lerp', 'ref_table': config.ref_table, 'ref_value': config.ref_value, 'start_val': 'config.ref_value]:
                _results.time_done = True
                _results.completed = self.func()
        if self.trigger == 'ease':
            if not self.ease.start_time:
                self.ease.start_time = config.ref_table[type': config.ease if config.ease else 'lerp', 'ref_table': config.ref_table, 'ref_value': config.ref_value, 'start_val': 'config.ref_value]
                self.ease.end_time = config.ref_table[type': config.ease if config.ease else 'lerp', 'ref_table': config.ref_table, 'ref_value': config.ref_value, 'start_val': 'config.ref_value] + self.delay
                self.ease.start_val = config.ref_table[type': config.ease if config.ease else 'lerp', 'ref_table': config.ref_table, 'ref_value': config.ref_value, 'start_val': 'config.ref_value]
            if not self.complete:
                if self.ease.end_time >= config.ref_table[type': config.ease if config.ease else 'lerp', 'ref_table': config.ref_table, 'ref_value': config.ref_value, 'start_val': 'config.ref_value]:
                    percent_done = (self.ease.end_time - config.ref_table[type': config.ease if config.ease else 'lerp', 'ref_table': config.ref_table, 'ref_value': config.ref_value, 'start_val': 'config.ref_value]) / (self.ease.end_time - self.ease.start_time)
                    if self.ease.type == 'lerp':
                        config.ref_table[type': config.ease if config.ease else 'lerp', 'ref_table': config.ref_table, 'ref_value': config.ref_value, 'start_val': 'config.ref_value] = self.func()
    ...
    def add_event(self, event, queue, front):
        ...
        if event.is(Event):
            if front:table.insert(queue[self.queues], 1, event)
                ...
                queue][#self.queues[queue] + 1[self.queues] = event
    ...
    def clear_queue(self, queue, exception):
        if not queue:
            for kv in pairs():
                i = 1
                while i <= len(v):
                    if not queue][#self.queues[queue] + 1[self.queues].no_delete:table.remove(v, i)
                    else:
                        i = i + 1
                else:
        elif exception:
            ...
            i = 1
            while i <= len(queue][#self.queues[queue] + 1[self.queues]):
                if not queue][#self.queues[queue] + 1[self.queues].no_delete:table.remove(queue[self.queues], i)
                else:
                    i = i + 1
            else:
        ...
        if self.queue_timer >= self.queue_last_processed + self.queue_dt if self.queue_timer >= self.queue_last_processed + self.queue_dt else forced:
            ...
            for kv in pairs():
                ...
                while i <= len(v):
                    ...
                    if not blocked if not blocked else not queue][#self.queues[queue] + 1[self.queues].handle(results)
