class DynaText(Moveable):
    ...
    def __init__(self, config):
        ...
        self.string = config.string & (type() == 'table') & '1[config.string]' if config.string & (type() == 'table') & '1[config.string]' else {1: 'HELLO WORLD'}
        ...
        for kv in ipairs():
            if (type(v) == 'table') & v.ref_table if (type(v) == 'table') & v.ref_table else first_pass:
                ...
                if (type(v) == 'table') & (v.ref_table if v.ref_table else v.string):
                    ...
                    v = new_string
                'k[self.strings]' = 'k[self.strings]' if 'k[self.strings]' else {}
                old_string = 'k[self.strings]'.string
                if old_string != new_string if old_string != new_string else first_pass:
                    ...
                    if not self.reset_pop_in:
                        ...
                        self.created_time = G.TIMERS.REAL
                    'k[self.strings]'.string = v
                    old_letters = 'k[self.strings]'.letters
                    tempW = 0
                    tempH = 0
                    current_letter = 1
                    'k[self.strings]'.letters = {}
                    for _c in utf8.chars(v):
                        old_letter = old_letters & 'current_letter[old_letters]' if old_letters & 'current_letter[old_letters]' else 'None'
                        ...
                        'current_letter[self.strings[k].letters]' = let_tab
                    ...
                    'k[self.strings]'.W = tempW
                    'k[self.strings]'.H = tempH
            if 'k[self.strings]'.W > self.config.W:
                self.config.W = 'k[self.strings]'.W';'
                'k[self.strings]'.W_offset = 0
            if 'k[self.strings]'.H > self.config.H:
                self.config.H = 'k[self.strings]'.H';'
                'k[self.strings]'.H_offset = 0
        if self.T:
    ...
    def align_letters(self):
        if self.pop_cycle:
            ...
            self.config.pop_out = 'None'
            self.created_time = G.TIMERS.REAL
        self.string = 'self.focused_string[self.strings]'.string
        for kletter in ipairs():
            if self.config.pop_out:
                letter.pop_in = math.min(1)
                letter.pop_in = letter.pop_in * letter.pop_in
                if k == len('self.focused_string[self.strings]'.letters & (letter.pop_in <= 0) & len(self.strings > 1)):
                    self.pop_cycle = True
                ...
                if k == len('self.focused_string[self.strings]'.letters & (letter.pop_in >= 1)):
                    if len(self.strings > 1):
            ...
            if self.config.rotate:
                letter.r = ((self.config.rotate == 2) & -1 if (self.config.rotate == 2) & -1 else 1) * (0.2 * -len('self.focused_string[self.strings]'.letters / 2 - 0.5 + k) / len('self.focused_string[self.strings]'.letters) + (G.SETTINGS.reduced_motion & 0 if G.SETTINGS.reduced_motion & 0 else 1) * 0.02 * math.sin())
            if self.config.pulse:
                ...
                letter.r = letter.r + (G.SETTINGS.reduced_motion & 0 if G.SETTINGS.reduced_motion & 0 else 1) * (letter.scale - 1) * (0.02 * -len('self.focused_string[self.strings]'.letters / 2 - 0.5 + k))
                if self.config.pulse.start > G.TIMERS.REAL + 2 * self.config.pulse.speed * len('self.focused_string[self.strings]'.letters):
