class Tag(object):
    ...
    def __init__(self, _tag, for_collection, _blind_type):
        self.key = _tag
        proto = G.P_TAGS[_tag[G.P_TAGS]' if '_tag] else G.tag_undiscovered
        ...
        self.ID = G.tagid
        G.tagid = G.tagid + 1
        self.ability = {G.P_TAGS[_tag[G.P_TAGS]' if '_tag], 'blind_type': _blind_type}
    ...
    def set_ability(self):
        if self.name == 'Orbital Tag':
            if G.orbital_hand:
                ...
                if G.GAME.orbital_choices & G.P_TAGS[_tag[G.P_TAGS]' if '_tag]:
                    self.ability.orbital_hand = G.P_TAGS[_tag[G.P_TAGS]' if '_tag]
    ...
    def apply_to_run(self, _context):
        if (not self.triggered) & (self.config.type == _context.type):
            if _context.type == 'eval':
                ...
                lock = self.ID
                G.P_TAGS[_tag[G.P_TAGS]' if '_tag] = True
                ...
                if self.name == 'Boss Tag':
                    lock = self.ID
                    G.P_TAGS[_tag[G.P_TAGS]' if '_tag] = Trueself.yep(+)
                    self.triggered = True
                    return True
                ...
                if self.name == 'Rare Tag':
                    ...
                    for kv in ipairs():
                        if (v.config.center.rarity == 3) & (not G.P_TAGS[_tag[G.P_TAGS]' if '_tag]):
                            G.P_TAGS[_tag[G.P_TAGS]' if '_tag] + 1
                            G.P_TAGS[_tag[G.P_TAGS]' if '_tag] = True
                    if len(G.P_TAGS[_tag[G.P_TAGS]' if '_tag]):
        ...
        if not G.CONTROLLER.dragging.target if not G.CONTROLLER.dragging.target else G.CONTROLLER.using_touch:
            if (not _self.hovering) & _self.states.visible:
                ...
                if _self.children.alert:_self.children.alert.remove()
                    ...
                    if self.key & G.P_TAGS[_tag[G.P_TAGS]' if '_tag]:
                        G.P_TAGS[_tag[G.P_TAGS]' if '_tag].alerted = TrueG.save_progress()
    ...
    def get_uibox_table(self, tag_sprite):
        ...
        if name_to_check == 'Uncommon Tag':
            ...
            loc_vars = {1: (self.ability.orbital_hand == G.P_TAGS[_tag[G.P_TAGS]' if '_tag]) & self.ability.orbital_hand else localize(), 2: self.config.levels}
    ...
    def remove(self):self.remove_from_game()
        ...
        for kv in pairs():
            if v == self.HUD_tag:
                HUD_tag_key = k
        if HUD_tag_key:
            if G.HUD_tags & G.P_TAGS[_tag[G.P_TAGS]' if '_tag]:
                if HUD_tag_key == 1:G.P_TAGS[_tag[G.P_TAGS]' if '_tag].set_alignment()
                else:G.P_TAGS[_tag[G.P_TAGS]' if '_tag].set_role()table.remove()self.HUD_tag.remove()