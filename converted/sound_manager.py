...
for _filename in ipairs(sound_files):
    ...
    for i in range(1, 1, ):
        if extension == '.ogg':LOAD_CHANNEL.push()
            ...
            'sound_code[SOURCES]' = {}table.insert(sound_code[SOURCES], s)
...
def MODULATE(args):
    ...
    for kv in pairs(SOURCES):
        i = 1
        while i <= len(v):
            if not 'i[v]'.sound.isPlaying():'i[v]'.sound.release()table.remove(v, i)
            else:
                i = i + 1
        else:
        for _s in pairs(v):
...
def AMBIENT(args):
    for kv in pairs(SOURCES):
        if 'k[args.ambient_control]':
            start_ambient = 'k[args.ambient_control]'.vol * (args.sound_settings.volume / 100.0) * (args.sound_settings.game_sounds_volume / 100.0) > 0
            for is in ipairs(v):
