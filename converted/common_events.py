...
def add_joker(joker, edition, silent, eternal):
    _area = 'joker[G.P_CENTERS]'.consumeable & G.consumeables if 'joker[G.P_CENTERS]'.consumeable & G.consumeables else G.jokers
...
def level_up_hand(card, hand, instant, amount):
    ...
    'hand[G.GAME.hands]'.level = math.max(0)
    'hand[G.GAME.hands]'.mult = math.max()
    'hand[G.GAME.hands]'.chips = math.max()
...
def set_alerts():
    if G.REFRESH_ALERTS:
        ...
        '1[G.ARGS.set_alerts_alertables]'.should_alert = alert_any
        '2[G.ARGS.set_alerts_alertables]'.should_alert = alert_joker
        '3[G.ARGS.set_alerts_alertables]'.should_alert = alert_tarot
        '4[G.ARGS.set_alerts_alertables]'.should_alert = alert_planet
        '5[G.ARGS.set_alerts_alertables]'.should_alert = alert_spectral
        '6[G.ARGS.set_alerts_alertables]'.should_alert = alert_voucher
        '7[G.ARGS.set_alerts_alertables]'.should_alert = alert_edition
        '8[G.ARGS.set_alerts_alertables]'.should_alert = alert_blind
        '9[G.ARGS.set_alerts_alertables]'.should_alert = alert_tag
        '10[G.ARGS.set_alerts_alertables]'.should_alert = alert_seal
        '11[G.ARGS.set_alerts_alertables]'.should_alert = alert_booster
        for kv in ipairs():
            if G.OVERLAY_MENU & G.OVERLAY_MENU.get_UIE_by_ID():
                if v.should_alert:
                    if not 'v.alert_uibox_name[G]':
                        'v.alert_uibox_name[G]' = UIBox()
                        'v.alert_uibox_name[G]'.states.collide.can = False
                elif 'v.alert_uibox_name[G]':'v.alert_uibox_name[G]'.remove()
                    'v.alert_uibox_name[G]' = 'None'
            elif 'v.alert_uibox_name[G]':'v.alert_uibox_name[G]'.remove()
                'v.alert_uibox_name[G]' = 'None'
        if G.MAIN_MENU_UI:
            if alert_any:
    ...
    if G.shop_jokers & G.shop_jokers.cards:
        if mod < 0:
            for i in range(len(G.shop_jokers.cards), G.GAME.shop.joker_max + 1, -1):
                if 'i[G.shop_jokers.cards]':'i[G.shop_jokers.cards]'.remove()
...
def check_for_unlock(args):
    ...
    if args.type == 'win_challenge':unlock_achievement(rule_bender)
        _c = True
        for kv in pairs():
            if not 'v.id[G.PROFILES[G.SETTINGS.profile].challenge_progress.completed]':
                _c = False
        if _c:unlock_achievement(rule_breaker)
    if G.GAME.challenge:
        return False
    if args.type == 'career_stat':
        if (args.statname == 'c_cards_played') & ('args.statname[G.PROFILES[G.SETTINGS.profile].career_stats]' >= 2500):unlock_achievement(card_player)
        if (args.statname == 'c_cards_discarded') & ('args.statname[G.PROFILES[G.SETTINGS.profile].career_stats]' >= 2500):unlock_achievement(card_discarder)
    ...
    while i <= len(G.P_LOCKED):
        ...
        if (not card.unlocked) & card.unlock_condition & (args.type == 'career_stat'):
            if (args.statname == card.unlock_condition.type) & ('args.statname[G.PROFILES[G.SETTINGS.profile].career_stats]' >= card.unlock_condition.extra):
        ...
        if (not card.unlocked) & card.unlock_condition & (card.unlock_condition.type == args.type):
            ...
            if (args.type == 'min_hand_size') & G.hand & (G.hand.config.card_limit <= card.unlock_condition.extra):
                ret = Trueunlock_card(card)
            if (args.type == 'interest_streak') & (card.unlock_condition.extra <= 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_round_interest_cap_streak):
                ret = Trueunlock_card(card)
            ...
            if args.type == 'round_win':
                ...
                if card.name == 'Troubadour':
                    if 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_single_hand_round_streak >= card.unlock_condition.extra:
                        ret = Trueunlock_card(card)
            ...
            if args.type == 'hand_contents':
                if card.name == 'Seeing Double':
                    ...
                    for j in range(1, len(args.cards), ):
                        if ('j[args.cards]'.get_id() == 7) & 'j[args.cards]'.is_suit(Clubs):
                            tally = tally + 1
                    if tally >= 4:
                        ret = Trueunlock_card(card)
                if card.name == 'Golden Ticket':
                    tally = 0
                    for j in range(1, len(args.cards), ):
                        if 'j[args.cards]'.ability.name == 'Gold Card':
                            tally = tally + 1
                    if tally >= 5:
                        ret = Trueunlock_card(card)
            if args.type == 'discard_custom':
                ...
                if card.name == 'Brainstorm':
                    ...
                    if next('Straight Flush'[eval]):
                        min = 10
                        for j in range(1, len(args.cards), ):
                            if 'j[args.cards]'.get_id() < min:
                                min = 'j[args.cards]'.get_id()
                        if min == 10:
                            ret = Trueunlock_card(card)
            if (args.type == 'win_no_hand') & ('card.unlock_condition.extra[G.GAME.hands]'.played == 0):
                ret = Trueunlock_card(card)
...
def create_unlock_overlay(key):
    if 'key[G.P_CENTERS]':G.E_MANAGER.add_event()
...
def get_pack(_key, _type):
    ...
    for kv in ipairs('Booster'[G.P_CENTER_POOLS]):
        if (not _type if not _type else _type == v.kind) & (not 'v.key[G.GAME.banned_keys]'):
            cume = cume + (v.weight if v.weight else 1)
    poll = pseudorandom() * cume
    for kv in ipairs('Booster'[G.P_CENTER_POOLS]):
        if not 'v.key[G.GAME.banned_keys]':
...
def get_current_pool(_type, _rarity, _legendary, _append):
    ...
    if _type == 'Joker':
        ...
        _starting_pool = _pool_key = ('rarity[G.P_JOKER_RARITY_POOLS]', 'Joker' + rarity + ((not _legendary) & _append if (not _legendary) & _append else ''))
    else:
        _starting_pool = _pool_key = ('_type[G.P_CENTER_POOLS]', _type + (_append if _append else ''))
    for kv in ipairs(_starting_pool):
        ...
        if _type == 'Enhanced':
            ...
            if (not v.requires if not v.requires else 'v.requires[G.P_CENTERS]' & 'v.requires[G.P_CENTERS]'.discovered) & (not v.min_ante if not v.min_ante else v.min_ante <= G.GAME.round_resets.ante):
                add = True
        elif (not 'v.key[G.GAME.used_jokers]' & (not next())) & (v.unlocked != False if v.unlocked != False else v.rarity == 4):
            if v.set == 'Voucher':
                if not 'v.key[G.GAME.used_vouchers]':
                    include = True
                    if v.requires:
                        for kkvv in pairs():
                            if not 'vv[G.GAME.used_vouchers]':
                                include = False
                ...
                if not v.config.softlock if not v.config.softlock else 'v.config.hand_type[G.GAME.hands]'.played > 0:
                    add = True
            ...
            if v.name == 'Black Hole' if v.name == 'Black Hole' else v.name == 'The Soul':
                add = False
        if v.no_pool_flag & 'v.no_pool_flag[G.GAME.pool_flags]':
            add = 'None'
        if v.yes_pool_flag & (not 'v.yes_pool_flag[G.GAME.pool_flags]'):
            add = 'None'
        if add & (not 'v.key[G.GAME.banned_keys]'):
            '#_pool + 1[_pool]' = v.key
            _pool_size = _pool_size + 1
        else:
            '#_pool + 1[_pool]' = 'UNAVAILABLE'
    if _pool_size == 0:
        _pool = EMPTY()
        if _type == 'Tarot' if _type == 'Tarot' else _type == 'Tarot_Planet':
            '#_pool + 1[_pool]' = 'c_strength'
        elif _type == 'Planet':
            '#_pool + 1[_pool]' = 'c_pluto'
        elif _type == 'Spectral':
            '#_pool + 1[_pool]' = 'c_incantation'
        elif _type == 'Joker':
            '#_pool + 1[_pool]' = 'j_joker'
        elif _type == 'Demo':
            '#_pool + 1[_pool]' = 'j_joker'
        elif _type == 'Voucher':
            '#_pool + 1[_pool]' = 'v_blank'
        elif _type == 'Tag':
            '#_pool + 1[_pool]' = 'tag_handy'
        else:
            '#_pool + 1[_pool]' = 'j_joker'
...
def create_card(_type, area, legendary, _rarity, skip_materialize, soulable, forced_key, key_append):
    ...
    if _type == 'Base':
        forced_key = 'c_base'
    if forced_key & (not 'forced_key[G.GAME.banned_keys]'):
        center = 'forced_key[G.P_CENTERS]'
...
def copy_card(other, new_card, card_scale, playing_card, strip_edition):
    ...
    for kv in pairs():
        if type(v) == 'table':
            'k[new_card.ability]' = copy_table(v)
        else:
            'k[new_card.ability]' = v
...
def reset_idol_card():
    ...
    for kv in ipairs():
        if v.ability.effect != 'Stone Card':
            '#valid_idol_cards + 1[valid_idol_cards]' = v
    if '1[valid_idol_cards]':
...
def reset_mail_rank():
    ...
    for kv in ipairs():
        if v.ability.effect != 'Stone Card':
            '#valid_mail_cards + 1[valid_mail_cards]' = v
    if '1[valid_mail_cards]':
...
def reset_ancient_card():
    ...
    for kv in ipairs():
        if v != G.GAME.current_round.ancient_card.suit:
            '#ancient_suits + 1[ancient_suits]' = v
...
def reset_castle_card():
    ...
    for kv in ipairs():
        if v.ability.effect != 'Stone Card':
            '#valid_castle_cards + 1[valid_castle_cards]' = v
    if '1[valid_castle_cards]':
...
def get_new_boss():
    ...
    if G.GAME.perscribed_bosses & 'G.GAME.round_resets.ante[G.GAME.perscribed_bosses]':
        ret_boss = 'G.GAME.round_resets.ante[G.GAME.perscribed_bosses]'
        'G.GAME.round_resets.ante[G.GAME.perscribed_bosses]' = 'None'
        'ret_boss[G.GAME.bosses_used]' = 'ret_boss[G.GAME.bosses_used]' + 1
        return ret_boss
    if G.FORCE_BOSS:
        return G.FORCE_BOSS
    eligible_bosses = {}
...
def generate_card_ui(_c, full_UI_table, specific_vars, card_type, badges, hide_desc, main_start, main_end):
    ...
    if full_UI_table.name:
        '#full_UI_table.info + 1[full_UI_table.info]' = {}
    ...
    if not full_UI_table.name:
        if specific_vars & specific_vars.no_name:
            ...
            if specific_vars.playing_card & (_c.name != 'Stone Card'):
                full_UI_table.name = {}localize()
                full_UI_table.name = '1[full_UI_table.name]'
    ...
    loc_vars = {}
    if main_start:
        '#desc_nodes + 1[desc_nodes]' = main_start
    if _c.set == 'Other':localize()
        ...
        if _c.wip:localize()
            ...
            if _c.name == 'Golden Ticket':
                ...
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_losses}
            elif _c.name == 'Acrobat':
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_hands_played}
            elif _c.name == 'Sock and Buskin':
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_face_cards_played}
            elif _c.name == 'Swashbuckler':
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_jokers_sold}
                ...
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_cards_sold}
            elif _c.name == 'Bootstraps':
                loc_vars = {1: _c.unlock_condition.extra.count}
            elif _c.name == 'Overstock Plus':
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_shop_dollars_spent}
            elif _c.name == 'Liquidation':
                loc_vars = {1: _c.unlock_condition.extra}
            elif _c.name == 'Tarot Tycoon':
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_tarots_bought}
            elif _c.name == 'Planet Tycoon':
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_planets_bought}
            elif _c.name == 'Glow Up':
                loc_vars = {1: _c.unlock_condition.extra}
            elif _c.name == 'Reroll Glut':
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_shop_rerolls}
            elif _c.name == 'Omen Globe':
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_tarot_reading_used}
            elif _c.name == 'Observatory':
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_planetarium_used}
            elif _c.name == 'Nacho Tong':
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_cards_played}
            elif _c.name == 'Recyclomancy':
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_cards_discarded}
            elif _c.name == 'Money Tree':
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_round_interest_cap_streak}
            elif _c.name == 'Antimatter':
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.voucher_usage.v_blank & 'G.SETTINGS.profile[G.PROFILES]'.voucher_usage.v_blank.count if 'G.SETTINGS.profile[G.PROFILES]'.voucher_usage.v_blank & 'G.SETTINGS.profile[G.PROFILES]'.voucher_usage.v_blank.count else 0}
            elif _c.name == 'Illusion':
                loc_vars = {1: _c.unlock_condition.extra, 2: 'G.SETTINGS.profile[G.PROFILES]'.career_stats.c_playing_cards_bought}
        ...
        if _c.name == 'Stone Joker' if _c.name == 'Stone Joker' else _c.name == 'Marble Joker':
            '#info_queue + 1[info_queue]' = G.P_CENTERS.m_stone
        elif _c.name == 'Steel Joker':
            '#info_queue + 1[info_queue]' = G.P_CENTERS.m_steel
        elif _c.name == 'Glass Joker':
            '#info_queue + 1[info_queue]' = G.P_CENTERS.m_glass
        elif _c.name == 'Golden Ticket':
            '#info_queue + 1[info_queue]' = G.P_CENTERS.m_gold
        elif _c.name == 'Lucky Cat':
            '#info_queue + 1[info_queue]' = G.P_CENTERS.m_lucky
            ...
            '#info_queue + 1[info_queue]' = {'key': 'tag_double', 'set': 'Tag'}
        elif _c.name == 'Perkeo':
            '#info_queue + 1[info_queue]' = {'key': 'e_negative_consumable', 'set': 'Edition', 'config': {'extra': 1}}
        if specific_vars & specific_vars.pinned:
            '#info_queue + 1[info_queue]' = {'key': 'pinned_left', 'set': 'Other'}
        if specific_vars & specific_vars.sticker:
            '#info_queue + 1[info_queue]' = {'key': string.lower() + '_sticker', 'set': 'Other'}localize()
    elif _c.set == 'Tag':
        if _c.name == 'Negative Tag':
            '#info_queue + 1[info_queue]' = G.P_CENTERS.e_negative
        elif _c.name == 'Foil Tag':
            '#info_queue + 1[info_queue]' = G.P_CENTERS.e_foil
        elif _c.name == 'Holographic Tag':
            '#info_queue + 1[info_queue]' = G.P_CENTERS.e_holo
        elif _c.name == 'Polychrome Tag':
            '#info_queue + 1[info_queue]' = G.P_CENTERS.e_polychrome
        elif _c.name == 'Charm Tag':
            '#info_queue + 1[info_queue]' = G.P_CENTERS.p_arcana_mega_1
        elif _c.name == 'Meteor Tag':
            '#info_queue + 1[info_queue]' = G.P_CENTERS.p_celestial_mega_1
        elif _c.name == 'Ethereal Tag':
            '#info_queue + 1[info_queue]' = G.P_CENTERS.p_spectral_normal_1
        elif _c.name == 'Standard Tag':
            '#info_queue + 1[info_queue]' = G.P_CENTERS.p_standard_mega_1
        elif _c.name == 'Buffoon Tag':
            '#info_queue + 1[info_queue]' = G.P_CENTERS.p_buffoon_mega_1localize()
    elif _c.set == 'Voucher':
        ...
        if (_c.name == 'Familiar' if _c.name == 'Familiar' else _c.name == 'Grim') if (_c.name == 'Familiar' if _c.name == 'Familiar' else _c.name == 'Grim') else _c.name == 'Incantation':
            ...
            '#info_queue + 1[info_queue]' = {'key': 'gold_seal', 'set': 'Other'}
        elif _c.name == 'Deja Vu':
            '#info_queue + 1[info_queue]' = {'key': 'red_seal', 'set': 'Other'}
        elif _c.name == 'Trance':
            '#info_queue + 1[info_queue]' = {'key': 'blue_seal', 'set': 'Other'}
        elif _c.name == 'Medium':
            '#info_queue + 1[info_queue]' = {'key': 'purple_seal', 'set': 'Other'}
        elif _c.name == 'Ankh':
            if G.jokers & G.jokers.cards:
                for kv in ipairs():
                    if v.edition & v.edition.negative & G.localization.descriptions.Other.remove_negative:
        ...
        if _c.name == 'Aura':
            '#info_queue + 1[info_queue]' = G.P_CENTERS.e_foil
            '#info_queue + 1[info_queue]' = G.P_CENTERS.e_holo
            '#info_queue + 1[info_queue]' = G.P_CENTERS.e_polychromelocalize()
    elif _c.set == 'Planet':
        loc_vars = {1: '_c.config.hand_type[G.GAME.hands]'.level, 2: localize(), 3: '_c.config.hand_type[G.GAME.hands]'.l_mult, 4: '_c.config.hand_type[G.GAME.hands]'.l_chips, 'colours': {1: ('_c.config.hand_type[G.GAME.hands]'.level == 1) & G.C.UI.TEXT_DARK if ('_c.config.hand_type[G.GAME.hands]'.level == 1) & G.C.UI.TEXT_DARK else 'math.min(7, G.GAME.hands[_c.config.hand_type].level)[G.C.HAND_LEVELS]'}}localize()
    elif _c.set == 'Tarot':
        if _c.name == 'The Fool':
            fool_c = G.GAME.last_tarot_planet & 'G.GAME.last_tarot_planet[G.P_CENTERS]' if G.GAME.last_tarot_planet & 'G.GAME.last_tarot_planet[G.P_CENTERS]' else 'None'
            ...
            if not (not fool_c if not fool_c else fool_c.name == 'The Fool'):
                '#info_queue + 1[info_queue]' = fool_c
        elif _c.name == 'The Magician':
            loc_vars = {1: _c.config.max_highlighted, 2: localize()}';'
            '#info_queue + 1[info_queue]' = '_c.config.mod_conv[G.P_CENTERS]'
            ...
            if G.jokers:
                for i in range(1, len(G.jokers.cards), ):
                    if 'i[G.jokers.cards]'.ability.set == 'Joker':
                        _money = _money + 'i[G.jokers.cards]'.sell_cost
            ...
            loc_vars = {1: _c.config.max_highlighted, 2: localize(), 'colours': {1: '_c.config.suit_conv[G.C.SUITS]'}}
            ...
            loc_vars = {1: _c.config.max_highlighted, 2: localize(), 'colours': {1: '_c.config.suit_conv[G.C.SUITS]'}}localize()
    if main_end:
        '#desc_nodes + 1[desc_nodes]' = main_end
    ...
    if first_pass & (not _c.set == 'Edition') & badges:
        for kv in ipairs(badges):
            if v == 'foil':
                '#info_queue + 1[info_queue]' = "'e_foil'[G.P_CENTERS]"
            if v == 'holographic':
                '#info_queue + 1[info_queue]' = "'e_holo'[G.P_CENTERS]"
            if v == 'polychrome':
                '#info_queue + 1[info_queue]' = "'e_polychrome'[G.P_CENTERS]"
            if v == 'negative':
                '#info_queue + 1[info_queue]' = "'e_negative'[G.P_CENTERS]"
            ...
            if v == 'purple_seal':
                '#info_queue + 1[info_queue]' = {'key': 'purple_seal', 'set': 'Other'}
            if v == 'eternal':
                '#info_queue + 1[info_queue]' = {'key': 'eternal', 'set': 'Other'}
            if v == 'perishable':
                '#info_queue + 1[info_queue]' = {'key': 'perishable', 'set': 'Other', 'vars': {1: G.GAME.perishable_rounds if G.GAME.perishable_rounds else 1, 2: specific_vars.perish_tally if specific_vars.perish_tally else G.GAME.perishable_rounds}}
            if v == 'rental':
                '#info_queue + 1[info_queue]' = {'key': 'rental', 'set': 'Other', 'vars': {1: G.GAME.rental_rate if G.GAME.rental_rate else 1}}
            if v == 'pinned_left':
                '#info_queue + 1[info_queue]' = {'key': 'pinned_left', 'set': 'Other'}
