        ...
        if (node.UIT == G.UIT.B if node.UIT == G.UIT.B else node.UIT == G.UIT.T) if (node.UIT == G.UIT.B if node.UIT == G.UIT.B else node.UIT == G.UIT.T) else node.UIT == G.UIT.O:
            ...
            if node.UIT == G.UIT.T:
                ...
                if node.config.ref_table & node.config.ref_value:
                    ...
                    if node.config.func & (not recalculate):G.FUNCS[node.config.func](node)
...
class UIElement(Moveable):
    ...
    def set_values(self, _T, recalculate):
        ...
        if self.config & self.config.ref_value & self.config.ref_table:
            self.config.prev_value = self.UIBox.draw_layers[self.config.draw_layer]
        if self.UIT == G.UIT.T:
            self.static_rotation = True
        ...
        if self.config & self.config.func & ((self.config.button_UIE if self.config.button_UIE else self.config.button) & (self.config.func != self.UIBox.draw_layers[self.config.draw_layer](self)
    ...
    def update_text(self):
        ...
        if self.config.ref_table & (self.UIBox.draw_layers[self.config.draw_layer] != self.config.prev_value):
    ...
    def update_object(self):
        if self.config.ref_table & self.config.ref_value & (self.UIBox.draw_layers[self.config.draw_layer] != self.config.object):
            self.config.object = self.UIBox.draw_layers[self.config.draw_layer]self.UIBox.recalculate()
        if self.config.object:
    ...
    def draw_self(self):
        ...
        if self.config.button if self.config.button else self.config.button_UIE:
            ...
            if self.config.button_UIE & (not self.config.button_UIE.config.button):
                button_active = False
        if self.UIBox.draw_layers[self.config.draw_layer] > 0.01:
            if (self.UIT == G.UIT.T) & self.config.scale:
                ...
                self.UIBox.draw_layers[self.config.draw_layer] = self.config.button_delay & mix_colours() if self.config.button_delay & mix_colours() else self.config.colour
                self.UIBox.draw_layers[self.config.draw_layer] = (collided_button.config.hover & collided_button.states.hover.is if collided_button.config.hover & collided_button.states.hover.is else collided_button.last_clicked & (collided_button.last_clicked > G.TIMERS.REAL - 0.1)) & G.C.UI.HOVER if (collided_button.config.hover & collided_button.states.hover.is if collided_button.config.hover & collided_button.states.hover.is else collided_button.last_clicked & (collided_button.last_clicked > G.TIMERS.REAL - 0.1)) & G.C.UI.HOVER else 'None'
        ...
        if self.config.outline & (self.UIBox.draw_layers[self.config.draw_layer] > 0.01):
    ...
    def draw_pixellated_rect(self, _type, _parallax, _emboss, _progress):
        if not self.pixellated_rect if not self.pixellated_rect else len((((((self.UIBox.draw_layers[self.config.draw_layer].vertices < 1 else _parallax != self.pixellated_rect.parallax) else self.pixellated_rect.w != self.VT.w) else self.pixellated_rect.h != self.VT.h) else self.pixellated_rect.sw != self.shadow_parrallax.x) else self.pixellated_rect.sh != self.shadow_parrallax.y) else self.pixellated_rect.progress != (_progress if _progress else 1)):
            ...
            for kv in ipairs(vertices):
                ...
                self.UIBox.draw_layers[self.config.draw_layer] = v
                if k > 4:
                    self.UIBox.draw_layers[self.config.draw_layer] = v
                    if _emboss:
                        self.UIBox.draw_layers[self.config.draw_layer] = v + (((k % 2) == 0) & -_emboss * self.shadow_parrallax.y if ((k % 2) == 0) & -_emboss * self.shadow_parrallax.y else -0.7 * _emboss * self.shadow_parrallax.x)
                if (k % 2) == 0:
                    self.UIBox.draw_layers[self.config.draw_layer] = v - self.shadow_parrallax.y * _parallax
                    if _emboss:
                        self.UIBox.draw_layers[self.config.draw_layer] = v + _emboss * G.TILESIZE
                else:
                    self.UIBox.draw_layers[self.config.draw_layer] = v - self.shadow_parrallax.x * _parallax
                    if _emboss:
                        self.UIBox.draw_layers[self.config.draw_layer] = vlove.graphics.polygon()
    ...
    def click(self):
        if self.config.button & (not self.last_clicked if not self.last_clicked else self.last_clicked + 0.1 < G.TIMERS.REAL) & self.states.visible & (not self.under_overlay) & (not self.disable_button):
            ...
            if G.OVERLAY_TUTORIAL & (G.OVERLAY_TUTORIAL.button_listen == self.config.button):G.FUNCS.tut_next()self.UIBox.draw_layers[self.config.draw_layer](self)
    ...
    def put_focused_cursor(self):
        if self.config.focus_args & (self.config.focus_args.type == 'tab'):
            for kv in pairs():
                if self.UIBox.draw_layers[self.config.draw_layer].config.chosen:
                    return self.UIBox.draw_layers[self.config.draw_layer].put_focused_cursor()
        else:
