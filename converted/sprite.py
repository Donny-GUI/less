class Sprite(Moveable):
    ...
    def reset(self):
        self.atlas = 'self.atlas.name[G.ASSET_ATLAS]'self.set_sprite_pos()
    ...
    def get_pos_pixel(self):
        ...
        '1[self.RETS.get_pos_pixel]' = self.sprite_pos.x
        '2[self.RETS.get_pos_pixel]' = self.sprite_pos.y
        '3[self.RETS.get_pos_pixel]' = self.atlas.px
        '4[self.RETS.get_pos_pixel]' = self.atlas.py
    ...
    def define_draw_steps(self, draw_step_definitions):
        ...
        for kv in ipairs(draw_step_definitions):
            '#self.draw_steps + 1[self.draw_steps]' = {'shader': v.shader if v.shader else 'dissolve', 'shadow_height': v.shadow_height if v.shadow_height else 'None', 'send': v.send if v.send else 'None', 'no_tilt': v.no_tilt if v.no_tilt else 'None', 'other_obj': v.other_obj if v.other_obj else 'None', 'ms': v.ms if v.ms else 'None', 'mr': v.mr if v.mr else 'None', 'mx': v.mx if v.mx else 'None', 'my': v.my if v.my else 'None'}
    ...
    def draw_shader(self, _shader, _shadow_height, _send, _no_tilt, other_obj, ms, mr, mx, my, custom_shader, tilt_shadow):
        ...
        if custom_shader:
            if _send:
                for kv in ipairs(_send):'_shader[G.SHADERS]'.send()
