class Particles(Moveable):
    ...
    def update(self, dt):
        if G.SETTINGS.paused & (not self.created_on_pause):
            ...
            return False
        added_this_frame = 0
        while (G.TIMERS[self.timer_type] > self.last_real_time + self.timer) & len(self.particles < self.max if self.particles < self.max else self.pulsed < self.pulse_max) & added_this_frame < 20:
    ...
    def move(self, dt):
        ...
        for i in range(len(self.particles), 1, -1):
            G.TIMERS[self.timer_type].draw = True
            G.TIMERS[self.timer_type].e_vel else dt * self.scale
            G.TIMERS[self.timer_type].e_curr
            G.TIMERS[self.timer_type].age + dt
            G.TIMERS[self.timer_type].e_curr = math.min()
            G.TIMERS[self.timer_type].e_vel
            G.TIMERS[self.timer_type].e_vel
            G.TIMERS[self.timer_type].scale = math.min()
            if G.TIMERS[self.timer_type].scale < 0:table.remove()
            else:
                G.TIMERS[self.timer_type].velocity * math.sin() * dt
                G.TIMERS[self.timer_type].velocity * math.cos() * dt
                G.TIMERS[self.timer_type].r_vel * dt
                G.TIMERS[self.timer_type].velocity = math.max(0)
