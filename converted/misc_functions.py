def GET_DISPLAYINFO(screenmode, display):
    ...
    for i in range(1, display_count, ):
        'i[G.SETTINGS.WINDOW.DISPLAYS]' = {}
        'i[G.SETTINGS.WINDOW.DISPLAYS]'.screen_resolutions = {'strings': {}, 'values': {}}
        'i[G.SETTINGS.WINDOW.display_names]' = '' + i
        render_w = render_h = love.window.getDesktopDimensions(i)
        unscaled_dims = '1[love.window.getFullscreenModes(i)]'
        'i[G.SETTINGS.WINDOW.DISPLAYS]'.DPI_scale = 1
        'i[G.SETTINGS.WINDOW.DISPLAYS]'.MONITOR_DIMS = unscaled_dims
        if screenmode == 'Fullscreen':
            for _v in ipairs():
                _w = _h = (v.width * 'i[G.SETTINGS.WINDOW.DISPLAYS]'.DPI_scale, v.height * 'i[G.SETTINGS.WINDOW.DISPLAYS]'.DPI_scale)
                if (_w <= 'i[G.SETTINGS.WINDOW.DISPLAYS]'.MONITOR_DIMS.width) & (_h <= 'i[G.SETTINGS.WINDOW.DISPLAYS]'.MONITOR_DIMS.height):
                    '#G.SETTINGS.WINDOW.DISPLAYS[i].screen_resolutions.strings + 1[G.SETTINGS.WINDOW.DISPLAYS[i].screen_resolutions.strings]' = '' + v.width + ' X ' + v.height
                    '#G.SETTINGS.WINDOW.DISPLAYS[i].screen_resolutions.values + 1[G.SETTINGS.WINDOW.DISPLAYS[i].screen_resolutions.values]' = {'w': v.width, 'h': v.height}
                    if (i == G.SETTINGS.WINDOW.selected_display) & (i == display) & (current_res_values.w == v.width) & (current_res_values.h == v.height):
                        res_option = len('i[G.SETTINGS.WINDOW.DISPLAYS]'.screen_resolutions.values)
        elif screenmode == 'Windowed':
            '1[G.SETTINGS.WINDOW.DISPLAYS[i].screen_resolutions.strings]' = '-'
            '1[G.SETTINGS.WINDOW.DISPLAYS[i].screen_resolutions.values]' = {'w': 1280, 'h': 720}
        elif screenmode == 'Borderless':
            '1[G.SETTINGS.WINDOW.DISPLAYS[i].screen_resolutions.strings]' = '' + 'i[G.SETTINGS.WINDOW.DISPLAYS]'.MONITOR_DIMS.width / 'i[G.SETTINGS.WINDOW.DISPLAYS]'.DPI_scale + ' X ' + 'i[G.SETTINGS.WINDOW.DISPLAYS]'.MONITOR_DIMS.height / 'i[G.SETTINGS.WINDOW.DISPLAYS]'.DPI_scale
            '1[G.SETTINGS.WINDOW.DISPLAYS[i].screen_resolutions.values]' = current_res_values
    return res_option
def timer_checkpoint(label, type, reset):
    ...
    if reset:
        ...
        cp.checkpoints = 0
        return False
    'cp.checkpoints + 1[cp.checkpoint_list]' = 'cp.checkpoints + 1[cp.checkpoint_list]' if 'cp.checkpoints + 1[cp.checkpoint_list]' else {}
    cp.checkpoints = cp.checkpoints + 1
    'cp.checkpoints[cp.checkpoint_list]'.label = label + ': ' + (collectgarbage(count) - G.PREV_GARB)
    'cp.checkpoints[cp.checkpoint_list]'.time = love.timer.getTime()
    'cp.checkpoints[cp.checkpoint_list]'.TTC = 'cp.checkpoints[cp.checkpoint_list]'.time - cp.last_time
    'cp.checkpoints[cp.checkpoint_list]'.trend = 'cp.checkpoints[cp.checkpoint_list]'.trend if 'cp.checkpoints[cp.checkpoint_list]'.trend else {}
    'cp.checkpoints[cp.checkpoint_list]'.states = 'cp.checkpoints[cp.checkpoint_list]'.states if 'cp.checkpoints[cp.checkpoint_list]'.states else {}table.insert()table.insert()
    '401[cp.checkpoint_list[cp.checkpoints].trend]' = 'None'
    '401[cp.checkpoint_list[cp.checkpoints].states]' = 'None'
    cp.last_time = 'cp.checkpoints[cp.checkpoint_list]'.time
    G.PREV_GARB = collectgarbage(count)
    av = 0
    for kv in ipairs():
        av = av + v / len('cp.checkpoints[cp.checkpoint_list]'.trend)
    'cp.checkpoints[cp.checkpoint_list]'.average = av
...
def pseudoshuffle(list, seed):
    ...
    if '1[list]' & '1[list]'.sort_id:table.sort(list)
    for i in range(len(list), 2, -1):
        j = math.random(i)
        'i[list]' = 'j[list]' = ('j[list]', 'i[list]')
    ...
    if G.GAME.stake >= 8:
        ...
        if (r_tally > 0) & (g_tally > 0):
            ...
            while not seed_found:
                ...
                if not 'get_first_legendary(seed_found)[r_leg]':
                    seed_found = 'None'
            else:
            return seed_found
...
def pseudorandom_element(_t, seed):
    ...
    for kv in pairs(_t):
        '#keys + 1[keys]' = {'k': k, 'v': v}
    if '1[keys]' & '1[keys]'.v & (type() == 'table') & '1[keys]'.v.sort_id:table.sort(keys)
    else:table.sort(keys)
    key = 'math.random(#keys)[keys]'.k
    return 'key[_t]'key
...
def pseudoseed(key, predict_seed):
    ...
    if not 'key[G.GAME.pseudorandom]':
        'key[G.GAME.pseudorandom]' = pseudohash()
    'key[G.GAME.pseudorandom]' = math.abs()
    return ('key[G.GAME.pseudorandom]' + (G.GAME.pseudorandom.hashed_seed if G.GAME.pseudorandom.hashed_seed else 0)) / 2
...
def get_blind_main_colour(blind):
    ...
    if (blind == 'Boss' if blind == 'Boss' else blind == 'Small') if (blind == 'Boss' if blind == 'Boss' else blind == 'Small') else blind == 'Big':
        ...
        if 'blind[G.GAME.round_resets.blind_states]' == 'Defeated' if 'blind[G.GAME.round_resets.blind_states]' == 'Defeated' else 'blind[G.GAME.round_resets.blind_states]' == 'Skipped':
            disabled = True
        blind = 'blind[G.GAME.round_resets.blind_choices]'
    return (((disabled if disabled else not 'blind[G.P_BLINDS]') & G.C.BLACK if (disabled if disabled else not 'blind[G.P_BLINDS]') & G.C.BLACK else 'blind[G.P_BLINDS]'.boss_colour) if ((disabled if disabled else not 'blind[G.P_BLINDS]') & G.C.BLACK if (disabled if disabled else not 'blind[G.P_BLINDS]') & G.C.BLACK else 'blind[G.P_BLINDS]'.boss_colour) else (blind == 'bl_small') & mix_colours() if (blind == 'bl_small') & mix_colours() else (blind == 'bl_big') & mix_colours()) if (((disabled if disabled else not 'blind[G.P_BLINDS]') & G.C.BLACK if (disabled if disabled else not 'blind[G.P_BLINDS]') & G.C.BLACK else 'blind[G.P_BLINDS]'.boss_colour) if ((disabled if disabled else not 'blind[G.P_BLINDS]') & G.C.BLACK if (disabled if disabled else not 'blind[G.P_BLINDS]') & G.C.BLACK else 'blind[G.P_BLINDS]'.boss_colour) else (blind == 'bl_small') & mix_colours() if (blind == 'bl_small') & mix_colours() else (blind == 'bl_big') & mix_colours()) else G.C.BLACK
def evaluate_poker_hand(hand):
    ...
    if next() & next():
        '"Flush Five"[results]' = parts._5
        if not results.top:
            results.top = '"Flush Five"[results]'
    if next() & next() & next():
        fh_hand = {}
        fh_3 = '1[parts._3]'
        fh_2 = '1[parts._2]'
        for i in range(1, len(fh_3), ):
            '#fh_hand + 1[fh_hand]' = 'i[fh_3]'
        for i in range(1, len(fh_2), ):
            '#fh_hand + 1[fh_hand]' = 'i[fh_2]'table.insert("Flush House"[results], fh_hand)
        if not results.top:
            results.top = '"Flush House"[results]'
    if next():
        '"Five of a Kind"[results]' = parts._5
        if not results.top:
            results.top = '"Five of a Kind"[results]'
    if next() & next():
        ...
        for _v in ipairs(1[_s]):
            ...
            if not in_straight:
                '#ret + 1[ret]' = v
        '"Straight Flush"[results]' = {1: ret}
        if not results.top:
            results.top = '"Straight Flush"[results]'
    if next():
        '"Four of a Kind"[results]' = parts._4
        if not results.top:
            results.top = '"Four of a Kind"[results]'
    if next() & next():
        fh_hand = {}
        fh_3 = '1[parts._3]'
        fh_2 = '1[parts._2]'
        for i in range(1, len(fh_3), ):
            '#fh_hand + 1[fh_hand]' = 'i[fh_3]'
        for i in range(1, len(fh_2), ):
            '#fh_hand + 1[fh_hand]' = 'i[fh_2]'table.insert("Full House"[results], fh_hand)
        if not results.top:
            results.top = '"Full House"[results]'
    if next():
        '"Flush"[results]' = parts._flush
        if not results.top:
            results.top = '"Flush"[results]'
    if next():
        '"Straight"[results]' = parts._straight
        if not results.top:
            results.top = '"Straight"[results]'
    if next():
        '"Three of a Kind"[results]' = parts._3
        if not results.top:
            results.top = '"Three of a Kind"[results]'
    if len(parts._2 == 2) if len(parts._2 == 2) else len((parts._3 == 1) & len(parts._2 == 1)):
        ...
        for i in range(1, len(fh_2a), ):
            '#fh_hand + 1[fh_hand]' = 'i[fh_2a]'
        for i in range(1, len(fh_2b), ):
            '#fh_hand + 1[fh_hand]' = 'i[fh_2b]'table.insert("Two Pair"[results], fh_hand)
        if not results.top:
            results.top = '"Two Pair"[results]'
    if next():
        '"Pair"[results]' = parts._2
        if not results.top:
            results.top = '"Pair"[results]'
    if next():
        '"High Card"[results]' = parts._highest
        if not results.top:
            results.top = '"High Card"[results]'
    if '1[results["Five of a Kind"]]':
        '"Four of a Kind"[results]' = {1: '1[results["Five of a Kind"]]', 2: '2[results["Five of a Kind"]]', 3: '3[results["Five of a Kind"]]', 4: '4[results["Five of a Kind"]]'}
    if '1[results["Four of a Kind"]]':
        '"Three of a Kind"[results]' = {1: '1[results["Four of a Kind"]]', 2: '2[results["Four of a Kind"]]', 3: '3[results["Four of a Kind"]]'}
    if '1[results["Three of a Kind"]]':
        '"Pair"[results]' = {1: '1[results["Three of a Kind"]]', 2: '2[results["Three of a Kind"]]'}
    return results
def get_flush(hand):
    ret = {}
    four_fingers = next()
    ...
    if len(hand > 5 if hand > 5 else len(hand < 5 - (four_fingers & 1 if four_fingers & 1 else 0))):
        ...
        for j in range(1, len(suits), ):
            ...
            for i in range(1, len(hand), ):
                if 'i[hand]'.is_suit(suit, None, True):
...
def get_X_same(num, hand):
    ...
    for i in range(len(hand), 1, -1):
        ...
        for j in range(1, len(hand), ):
            if ('i[hand]'.get_id() == 'j[hand]'.get_id()) & (i != j):table.insert(curr, j[hand])
        if len(curr == num):
            'curr[1]:get_id()[vals]' = curr
    ret = {}
...
def add_to_drawhash(obj):
    if obj:
        '#G.DRAW_HASH + 1[G.DRAW_HASH]' = obj
def mix_colours(C1, C2, proportionC1):
    return {1: ('1[C1]' if '1[C1]' else 0.5) * proportionC1 + ('1[C2]' if '1[C2]' else 0.5) * (1 - proportionC1), 2: ('2[C1]' if '2[C1]' else 0.5) * proportionC1 + ('2[C2]' if '2[C2]' else 0.5) * (1 - proportionC1), 3: ('3[C1]' if '3[C1]' else 0.5) * proportionC1 + ('3[C2]' if '3[C2]' else 0.5) * (1 - proportionC1), 4: ('4[C1]' if '4[C1]' else 1) * proportionC1 + ('4[C2]' if '4[C2]' else 1) * (1 - proportionC1)}
...
def modulate_sound(dt):
    ...
    for kv in pairs():
        'k[AC]' = 'k[AC]' if 'k[AC]' else {}
        'k[AC]'.per = (((k == 'ambientOrgan1') & 0.7 if (k == 'ambientOrgan1') & 0.7 else (k == 'ambientFire1') & 1.1) if ((k == 'ambientOrgan1') & 0.7 if (k == 'ambientOrgan1') & 0.7 else (k == 'ambientFire1') & 1.1) else (k == 'ambientFire2') & 1.05) if (((k == 'ambientOrgan1') & 0.7 if (k == 'ambientOrgan1') & 0.7 else (k == 'ambientFire1') & 1.1) if ((k == 'ambientOrgan1') & 0.7 if (k == 'ambientOrgan1') & 0.7 else (k == 'ambientFire1') & 1.1) else (k == 'ambientFire2') & 1.05) else 1
        'k[AC]'.vol = ((not G.video_organ) & (G.STATE == G.STATES.SPLASH) & 0 if (not G.video_organ) & (G.STATE == G.STATES.SPLASH) & 0 else 'k[AC]'.vol & v.volfunc()) if ((not G.video_organ) & (G.STATE == G.STATES.SPLASH) & 0 if (not G.video_organ) & (G.STATE == G.STATES.SPLASH) & 0 else 'k[AC]'.vol & v.volfunc()) else 0
...
def lighten(colour, percent, no_tab):
    if no_tab:
        return '1[colour]' * (1 - percent) + percent'2[colour]' * (1 - percent) + percent'3[colour]' * (1 - percent) + percent'4[colour]'
    return {1: '1[colour]' * (1 - percent) + percent, 2: '2[colour]' * (1 - percent) + percent, 3: '3[colour]' * (1 - percent) + percent, 4: '4[colour]'}
def darken(colour, percent, no_tab):
    if no_tab:
        return '1[colour]' * (1 - percent)'2[colour]' * (1 - percent)'3[colour]' * (1 - percent)'4[colour]'
    return {1: '1[colour]' * (1 - percent), 2: '2[colour]' * (1 - percent), 3: '3[colour]' * (1 - percent), 4: '4[colour]'}
def adjust_alpha(colour, new_alpha, no_tab):
    if no_tab:
        return '1[colour]''2[colour]''3[colour]'new_alpha
    return {1: '1[colour]', 2: '2[colour]', 3: '3[colour]', 4: new_alpha}
def alert_no_space(card, area):
    G.CONTROLLER.locks.no_space = Trueattention_text()card.juice_up(0.3, 0.2)
    for i in range(1, len(area.cards), ):'i[area.cards]'.juice_up(0.15)G.E_MANAGER.add_event()play_sound(tarot2, 1, 0.4)G.E_MANAGER.add_event()
...
def get_blind_amount(ante):
    ...
    if not G.GAME.modifiers.scaling if not G.GAME.modifiers.scaling else G.GAME.modifiers.scaling == 1:
        ...
        if ante <= 8:
            return 'ante[amounts]'
        a = b = c = d = ('8[amounts]', 1.6, ante - 8, 1 + 0.2 * (ante - 8))
        amount = math.floor()
...
def copy_table(O):
    ...
    if O_type == 'table':
        ...
        for kv in nextO'None':
            'copy_table(k)[copy]' = copy_table(v)setmetatable(copy)
    else:
        copy = O
    return copy
...
def check_and_set_high_score(score, amt):
    if not amt if not amt else type(amt) != 'number':
        return False
    if 'score[G.GAME.round_scores]' & (math.floor(amt) > 'score[G.GAME.round_scores]'.amt):
        'score[G.GAME.round_scores]'.amt = math.floor(amt)
    if G.GAME.seeded:
        return False
    ...
    if 'score[G.PROFILES[G.SETTINGS.profile].high_scores]' & (math.floor(amt) > 'score[G.PROFILES[G.SETTINGS.profile].high_scores]'.amt):
        if 'score[G.GAME.round_scores]':
            'score[G.GAME.round_scores]'.high_score = True
        'score[G.PROFILES[G.SETTINGS.profile].high_scores]'.amt = math.floor(amt)G.save_settings()
def set_joker_usage():
    for kv in pairs():
        if v.config.center_key & (v.ability.set == 'Joker'):
            if 'v.config.center_key[G.PROFILES[G.SETTINGS.profile].joker_usage]':
                'v.config.center_key[G.PROFILES[G.SETTINGS.profile].joker_usage]'.count = 'v.config.center_key[G.PROFILES[G.SETTINGS.profile].joker_usage]'.count + 1
            else:
                'v.config.center_key[G.PROFILES[G.SETTINGS.profile].joker_usage]' = {'count': 1, 'order': v.config.center.order, 'wins': {}, 'losses': {}}G.save_settings()
def set_joker_win():
    for kv in pairs():
        if v.config.center_key & (v.ability.set == 'Joker'):
            'v.config.center_key[G.PROFILES[G.SETTINGS.profile].joker_usage]' = 'v.config.center_key[G.PROFILES[G.SETTINGS.profile].joker_usage]' if 'v.config.center_key[G.PROFILES[G.SETTINGS.profile].joker_usage]' else {'count': 1, 'order': v.config.center.order, 'wins': {}, 'losses': {}}
            if 'v.config.center_key[G.PROFILES[G.SETTINGS.profile].joker_usage]':
                'v.config.center_key[G.PROFILES[G.SETTINGS.profile].joker_usage]'.wins = 'v.config.center_key[G.PROFILES[G.SETTINGS.profile].joker_usage]'.wins if 'v.config.center_key[G.PROFILES[G.SETTINGS.profile].joker_usage]'.wins else {}
                'G.GAME.stake[G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].wins]' = ('G.GAME.stake[G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].wins]' if 'G.GAME.stake[G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].wins]' else 0) + 1G.save_settings()
def get_joker_win_sticker(_center, index):
    if '_center.key[G.PROFILES[G.SETTINGS.profile].joker_usage]' & '_center.key[G.PROFILES[G.SETTINGS.profile].joker_usage]'.wins:
        _w = 0
        for kv in pairs():
            _w = math.max(k, _w)
        if index:
...
def set_joker_loss():
    for kv in pairs():
        if v.config.center_key & (v.ability.set == 'Joker'):
            if 'v.config.center_key[G.PROFILES[G.SETTINGS.profile].joker_usage]':
                'v.config.center_key[G.PROFILES[G.SETTINGS.profile].joker_usage]'.losses = 'v.config.center_key[G.PROFILES[G.SETTINGS.profile].joker_usage]'.losses if 'v.config.center_key[G.PROFILES[G.SETTINGS.profile].joker_usage]'.losses else {}
                'G.GAME.stake[G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].losses]' = ('G.GAME.stake[G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].losses]' if 'G.GAME.stake[G.PROFILES[G.SETTINGS.profile].joker_usage[v.config.center_key].losses]' else 0) + 1G.save_settings()
def set_deck_usage():
    if G.GAME.selected_back & G.GAME.selected_back.effect & G.GAME.selected_back.effect.center & G.GAME.selected_back.effect.center.key:
        ...
        if 'deck_key[G.PROFILES[G.SETTINGS.profile].deck_usage]':
            'deck_key[G.PROFILES[G.SETTINGS.profile].deck_usage]'.count = 'deck_key[G.PROFILES[G.SETTINGS.profile].deck_usage]'.count + 1
        else:
            'deck_key[G.PROFILES[G.SETTINGS.profile].deck_usage]' = {'count': 1, 'order': G.GAME.selected_back.effect.center.order, 'wins': {}, 'losses': {}}G.save_settings()
def set_deck_win():
    if G.GAME.selected_back & G.GAME.selected_back.effect & G.GAME.selected_back.effect.center & G.GAME.selected_back.effect.center.key:
        deck_key = G.GAME.selected_back.effect.center.key
        if not 'deck_key[G.PROFILES[G.SETTINGS.profile].deck_usage]':
            'deck_key[G.PROFILES[G.SETTINGS.profile].deck_usage]' = {'count': 1, 'order': G.GAME.selected_back.effect.center.order, 'wins': {}, 'losses': {}}
        if 'deck_key[G.PROFILES[G.SETTINGS.profile].deck_usage]':
            'G.GAME.stake[G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].wins]' = ('G.GAME.stake[G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].wins]' if 'G.GAME.stake[G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].wins]' else 0) + 1
            for i in range(1, G.GAME.stake, ):
                'i[G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].wins]' = 'i[G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].wins]' if 'i[G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].wins]' else 1set_challenge_unlock()G.save_settings()
def set_challenge_unlock():
    if 'G.SETTINGS.profile[G.PROFILES]'.all_unlocked:
        return False
    if 'G.SETTINGS.profile[G.PROFILES]'.challenges_unlocked:
        _ch_comp = _ch_tot = (0, len(G.CHALLENGES))
        for kv in ipairs():
            if v.id & "v.id or ''[G.PROFILES[G.SETTINGS.profile].challenge_progress.completed]":
                _ch_comp = _ch_comp + 1
        'G.SETTINGS.profile[G.PROFILES]'.challenges_unlocked = math.min(_ch_tot)
    else:
        deck_wins = 0
        for kv in pairs():
            if v.wins & '1[v.wins]':
                deck_wins = deck_wins + 1
        if (deck_wins >= G.CHALLENGE_WINS) & (not 'G.SETTINGS.profile[G.PROFILES]'.challenges_unlocked):
            'G.SETTINGS.profile[G.PROFILES]'.challenges_unlocked = 5notify_alert(b_challenge, Back)
def get_deck_win_stake(_deck_key):
    ...
    if '_deck_key[G.PROFILES[G.SETTINGS.profile].deck_usage]' & '_deck_key[G.PROFILES[G.SETTINGS.profile].deck_usage]'.wins:
        _w = 0
        for kv in pairs():
            _w = math.max(k, _w)
        return _w
...
def get_deck_win_sticker(_center):
    if '_center.key[G.PROFILES[G.SETTINGS.profile].deck_usage]' & '_center.key[G.PROFILES[G.SETTINGS.profile].deck_usage]'.wins:
        _w = -1
        for kv in pairs():
            _w = math.max(k, _w)
        if _w > 0:
...
def set_deck_loss():
    if G.GAME.selected_back & G.GAME.selected_back.effect & G.GAME.selected_back.effect.center & G.GAME.selected_back.effect.center.key:
        ...
        if not 'deck_key[G.PROFILES[G.SETTINGS.profile].deck_usage]':
            'deck_key[G.PROFILES[G.SETTINGS.profile].deck_usage]' = {'count': 1, 'order': G.GAME.selected_back.effect.center.order, 'wins': {}, 'losses': {}}
        if 'deck_key[G.PROFILES[G.SETTINGS.profile].deck_usage]':
            'G.GAME.stake[G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].losses]' = ('G.GAME.stake[G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].losses]' if 'G.GAME.stake[G.PROFILES[G.SETTINGS.profile].deck_usage[deck_key].losses]' else 0) + 1G.save_settings()
def set_consumeable_usage(card):
    if card.config.center_key & card.ability.consumeable:
        if 'card.config.center_key[G.PROFILES[G.SETTINGS.profile].consumeable_usage]':
            'card.config.center_key[G.PROFILES[G.SETTINGS.profile].consumeable_usage]'.count = 'card.config.center_key[G.PROFILES[G.SETTINGS.profile].consumeable_usage]'.count + 1
        else:
            'card.config.center_key[G.PROFILES[G.SETTINGS.profile].consumeable_usage]' = {'count': 1, 'order': card.config.center.order}
        if 'card.config.center_key[G.GAME.consumeable_usage]':
            'card.config.center_key[G.GAME.consumeable_usage]'.count = 'card.config.center_key[G.GAME.consumeable_usage]'.count + 1
        else:
            'card.config.center_key[G.GAME.consumeable_usage]' = {'count': 1, 'order': card.config.center.order, 'set': card.ability.set}
...
def set_voucher_usage(card):
    if card.config.center_key & (card.ability.set == 'Voucher'):
        if 'card.config.center_key[G.PROFILES[G.SETTINGS.profile].voucher_usage]':
            'card.config.center_key[G.PROFILES[G.SETTINGS.profile].voucher_usage]'.count = 'card.config.center_key[G.PROFILES[G.SETTINGS.profile].voucher_usage]'.count + 1
        else:
            'card.config.center_key[G.PROFILES[G.SETTINGS.profile].voucher_usage]' = {'count': 1, 'order': card.config.center.order}G.save_settings()
def set_hand_usage(hand):
    hand_label = hand
    hand = hand.gsub(%s+, )
    if 'hand[G.PROFILES[G.SETTINGS.profile].hand_usage]':
        'hand[G.PROFILES[G.SETTINGS.profile].hand_usage]'.count = 'hand[G.PROFILES[G.SETTINGS.profile].hand_usage]'.count + 1
    else:
        'hand[G.PROFILES[G.SETTINGS.profile].hand_usage]' = {'count': 1, 'order': hand_label}
    if 'hand[G.GAME.hand_usage]':
        'hand[G.GAME.hand_usage]'.count = 'hand[G.GAME.hand_usage]'.count + 1
    else:
        'hand[G.GAME.hand_usage]' = {'count': 1, 'order': hand_label}G.save_settings()
def set_profile_progress():
    ...
    for _v in pairs():
        ...
        if 'v.id[G.PROFILES[G.SETTINGS.profile].challenge_progress.completed]':
            G.PROGRESS.challenges.tally = G.PROGRESS.challenges.tally + 1
    'G.SETTINGS.profile[G.PROFILES]'.progress.joker_stickers = copy_table()
    'G.SETTINGS.profile[G.PROFILES]'.progress.deck_stakes = copy_table()
    'G.SETTINGS.profile[G.PROFILES]'.progress.challenges = copy_table()
def set_discover_tallies():
    ...
    for _v in pairs():
        ...
        if v.discovered:
            ...
            G.DISCOVER_TALLIES.total.tally = G.DISCOVER_TALLIES.total.tally + 1
    'G.SETTINGS.profile[G.PROFILES]'.high_scores.collection.amt = G.DISCOVER_TALLIES.total.tally
    'G.SETTINGS.profile[G.PROFILES]'.high_scores.collection.tot = G.DISCOVER_TALLIES.total.of
    'G.SETTINGS.profile[G.PROFILES]'.progress.discovered = copy_table()
...
def inc_career_stat(stat, mod):
    if G.GAME.seeded if G.GAME.seeded else G.GAME.challenge:
        return False
    if not 'stat[G.PROFILES[G.SETTINGS.profile].career_stats]':
        'stat[G.PROFILES[G.SETTINGS.profile].career_stats]' = 0
    'stat[G.PROFILES[G.SETTINGS.profile].career_stats]' = 'stat[G.PROFILES[G.SETTINGS.profile].career_stats]' + (mod if mod else 0)G.save_settings()
def recursive_table_cull(t):
    ret_t = {}
    for kv in pairs(t):
        if type(v) == 'table':
            if v.is & v.is(Object):
                'k[ret_t]' = '"' + 'MANUAL_REPLACE' + '"'
            else:
                'k[ret_t]' = recursive_table_cull(v)
        else:
            'k[ret_t]' = v
    return ret_t
...
def loc_colour(_c, _default):
    G.ARGS.LOC_COLOURS = G.ARGS.LOC_COLOURS if G.ARGS.LOC_COLOURS else {'red': G.C.RED, 'mult': G.C.MULT, 'blue': G.C.BLUE, 'chips': G.C.CHIPS, 'green': G.C.GREEN, 'money': G.C.MONEY, 'gold': G.C.GOLD, 'attention': G.C.FILTER, 'purple': G.C.PURPLE, 'white': G.C.WHITE, 'inactive': G.C.UI.TEXT_INACTIVE, 'spades': G.C.SUITS.Spades, 'hearts': G.C.SUITS.Hearts, 'clubs': G.C.SUITS.Clubs, 'diamonds': G.C.SUITS.Diamonds, 'tarot': G.C.SECONDARY_SET.Tarot, 'planet': G.C.SECONDARY_SET.Planet, 'spectral': G.C.SECONDARY_SET.Spectral, 'edition': G.C.EDITION, 'dark_edition': G.C.DARK_EDITION, 'legendary': '4[G.C.RARITY]', 'enhanced': G.C.SECONDARY_SET.Enhanced}
    return ('_c[G.ARGS.LOC_COLOURS]' if '_c[G.ARGS.LOC_COLOURS]' else _default) if ('_c[G.ARGS.LOC_COLOURS]' if '_c[G.ARGS.LOC_COLOURS]' else _default) else G.C.UI.TEXT_DARK
def init_localization():
    G.localization.misc.v_dictionary_parsed = {}
    for kv in pairs():
        if type(v) == 'table':
            'k[G.localization.misc.v_dictionary_parsed]' = {'multi_line': True}
            for kkvv in ipairs(v):
                'kk[G.localization.misc.v_dictionary_parsed[k]]' = loc_parse_string(vv)
        else:
            'k[G.localization.misc.v_dictionary_parsed]' = loc_parse_string(v)
    G.localization.misc.v_text_parsed = {}
    for kv in pairs():
        'k[G.localization.misc.v_text_parsed]' = {}
        for kkvv in ipairs(v):
            'kk[G.localization.misc.v_text_parsed[k]]' = loc_parse_string(vv)
    G.localization.tutorial_parsed = {}
    for kv in pairs():
        'k[G.localization.tutorial_parsed]' = {'multi_line': True}
        for kkvv in ipairs(v):
            'kk[G.localization.tutorial_parsed[k]]' = loc_parse_string(vv)
    G.localization.quips_parsed = {}
    for kv in pairs():
        'k[G.localization.quips_parsed]' = {'multi_line': True}
        for kkvv in ipairs(v):
            'kk[G.localization.quips_parsed[k]]' = loc_parse_string(vv)
            ...
            for _set in pairs(group):
                for _center in pairs(set):
                    ...
                    if not center.text:
                        ...
                        for _line in ipairs():
                            '#center.text_parsed + 1[center.text_parsed]' = loc_parse_string(line)
                        center.name_parsed = {}
                        for _line in ipairs():
                            '#center.name_parsed + 1[center.name_parsed]' = loc_parse_string(line)
                        if center.unlock:
                            center.unlock_parsed = {}
                            for _line in ipairs():
                                '#center.unlock_parsed + 1[center.unlock_parsed]' = loc_parse_string(line)
    ...
    if love.filesystem.getInfo():
        ...
        if love.filesystem.getInfo():
            for line in string.gmatch():
                key = line.gsub(%s+, )
                if key & (key != ''):
                    'key[_meta.unlocked]' = True
        if love.filesystem.getInfo():
            for line in string.gmatch():
                key = line.gsub(%s+, )
                if key & (key != ''):
                    'key[_meta.discovered]' = True
        if love.filesystem.getInfo():
            for line in string.gmatch():
                key = line.gsub(%s+, )
                if key & (key != ''):
                    'key[_meta.alerted]' = Truelove.filesystem.remove()love.filesystem.remove()love.filesystem.remove()compress_and_save()
...
def loc_parse_string(line):
    ...
    for i in range(1, len(line), ):
        ...
        if char == '{':
            if '1[str_parts]':
                '#parsed_line + 1[parsed_line]' = {'strings': str_parts, 'control': control if control else {}}
            str_parts = str_it = ({}, 1)
            ...
            _c_gather = 'None'';'
            if _c_name:
                '_c_name[control]' = _c_val';'
            _c_name = 'None'';'
            _c_val = 'None'';'
            if char == '}':
                _c = 'None'
        elif (not _c) & (char != '#') & (not _s_gather):
            'str_it[str_parts]' = ('str_it[str_parts]' if 'str_it[str_parts]' else '') + ("'X'[control]" & char.gsub(%s+, ) if "'X'[control]" & char.gsub(%s+, ) else char)
        elif (not _c) & (char == '#') & (not _s_gather):
            _s_gather = True';'
            if 'str_it[str_parts]':
                str_it = str_it + 1
...
def localize(args, misc_cat):
    if args & (not type(args) == 'table'):
        if misc_cat & 'misc_cat[G.localization.misc]':
            return 'args[G.localization.misc[misc_cat]]' if 'args[G.localization.misc[misc_cat]]' else 'ERROR'
        return 'args[G.localization.misc.dictionary]' if 'args[G.localization.misc.dictionary]' else 'ERROR'
    loc_target = 'None'
    ret_string = 'None'
    if args.type == 'other':
        loc_target = 'args.key[G.localization.descriptions.Other]'
    elif args.type == 'descriptions' if args.type == 'descriptions' else args.type == 'unlocks':
        loc_target = 'args.key[G.localization.descriptions[args.set]]'
    elif args.type == 'tutorial':
        loc_target = 'args.key[G.localization.tutorial_parsed]'
    elif args.type == 'quips':
        loc_target = 'args.key[G.localization.quips_parsed]'
            ...
            for _lines in ipairs():
                ...
                for _part in ipairs(lines):
                    ...
                    for _subpart in ipairs():
                        assembled_string = assembled_string + (((type(subpart) == 'string') & subpart if (type(subpart) == 'string') & subpart else 'tonumber(subpart[1])[args.vars]') if ((type(subpart) == 'string') & subpart if (type(subpart) == 'string') & subpart else 'tonumber(subpart[1])[args.vars]') else 'ERROR')
                    final_line = final_line + assembled_string
                '#multi_line + 1[multi_line]' = final_line
        return multi_line
    elif args.type == 'text':
        loc_target = 'args.key[G.localization.misc.v_text_parsed]'
    elif args.type == 'variable':
        loc_target = 'args.key[G.localization.misc.v_dictionary_parsed]'
        if not loc_target:
            return 'ERROR'
        if loc_target.multi_line:
            ...
            for kv in ipairs(loc_target):
                assembled_string = ''
                for _subpart in ipairs():
                    assembled_string = assembled_string + ((type(subpart) == 'string') & subpart if (type(subpart) == 'string') & subpart else 'tonumber(subpart[1])[args.vars]')
                'k[assembled_strings]' = assembled_string
        ...
        loc_target = 'args.key or args.node.config.center.key[G.localization.descriptions[args.set or args.node.config.center.set]]'
    if ret_string:
        return ret_string
    if loc_target:
        for _lines in ipairs():
            ...
            for _part in ipairs(lines):
                ...
                if args.type == 'name':
                    '#final_line + 1[final_line]' = {'n': G.UIT.O, 'config': {'object': DynaText()}}
                elif part.control.E:
                    ...
                    '#final_line + 1[final_line]' = {'n': G.UIT.C, 'config': {'align': 'm', 'colour': loc_colour(), 'r': 0.05, 'padding': 0.03, 'res': 0.15}, 'nodes': {1: {'n': G.UIT.T, 'config': {'text': assembled_string, 'colour': loc_colour(), 'scale': 0.32 * (part.control.s & tonumber() if part.control.s & tonumber() else 1) * desc_scale}}}}
                else:
                    '#final_line + 1[final_line]' = {'n': G.UIT.T, 'config': {'detailed_tooltip': part.control.T & ('part.control.T[G.P_CENTERS]' if 'part.control.T[G.P_CENTERS]' else 'part.control.T[G.P_TAGS]') if part.control.T & ('part.control.T[G.P_CENTERS]' if 'part.control.T[G.P_CENTERS]' else 'part.control.T[G.P_TAGS]') else 'None', 'text': assembled_string, 'shadow': args.shadow, 'colour': part.control.V & 'tonumber(part.control.V)[args.vars.colours]' if part.control.V & 'tonumber(part.control.V)[args.vars.colours]' else loc_colour(), 'scale': 0.32 * (part.control.s & tonumber() if part.control.s & tonumber() else 1) * desc_scale}}
            if args.type == 'name' if args.type == 'name' else args.type == 'text':
                return final_line
            '#args.nodes + 1[args.nodes]' = final_line
def get_stake_sprite(_stake, _scale):
    ...
    if _stake == 8:
        ...
        '1[_sprite.ARGS.send_to_shader]' = math.min() + G.TIMERS.REAL / 18 + (_sprite.juice & _sprite.juice.r * 20 if _sprite.juice & _sprite.juice.r * 20 else 0) + 1
        '2[_sprite.ARGS.send_to_shader]' = G.TIMERS.REALSprite.draw_shader(_sprite, dissolve)Sprite.draw_shader(_sprite, voucher, None)
    return stake_sprite
...
def get_challenge_rule(_challenge, _type, _id):
    if _challenge & _challenge.rules & '_type[_challenge.rules]':
...
def PLAY_SOUND(args):
    ...
    'args.sound_code[SOURCES]' = 'args.sound_code[SOURCES]' if 'args.sound_code[SOURCES]' else {}
...
def MODULATE(args):
    for kv in pairs(SOURCES):
        if string.find(k, music) & (args.desired_track != ''):
            if '1[v]' & '1[v]'.sound & '1[v]'.sound.isPlaying():
            else:RESTART_MUSIC(args)
                break';'
    for kv in pairs(SOURCES):
        i = 1
        while i <= len(v):
            if not 'i[v]'.sound.isPlaying():table.remove(v, i)
            else:
                i = i + 1
        else:
        for is in ipairs(v):
...
def AMBIENT(args):
    for kv in pairs(SOURCES):
        if 'k[args.ambient_control]':
            start_ambient = 'k[args.ambient_control]'.vol > 0
            for is in ipairs(v):
                if s.sound & s.sound.isPlaying() & s.original_volume:
                    s.original_volume = 'k[args.ambient_control]'.volSET_SFX(s, args)
                    start_ambient = False
            if start_ambient:
                args.sound_code = k
                args.vol = 'k[args.ambient_control]'.vol
                args.per = 'k[args.ambient_control]'.perPLAY_SOUND(args)
